...
export function CustomerSignupPage() {
  ...
  const checkPhone = async () => {
    ...
    try {
      ...
      if (verificationStatus === 'TEST_READY') { const code=window.prompt('NICE 테스트 인증코드를 입력해 주세요.'); if(!code)return; const result=await customerAccountApi.completeTestIdentityVerification(form.phone,code); setPhoneChecked(true); setForm(current=>({...current,phoneVerificationToken:result.verificationToken})); setError(''); return }
  ...
  return <div className="signupShell"><header><BrandLogo /><button onClick={() => navigate('/login')}>로그인</button></header><main className="signupMain"><ol className="signupSteps" aria-label="회원가입 단계">{['휴대전화 확인', '계정정보', '약관동의', '정보확인', '가입완료'].map((label, index) => <li className={step >= index + 1 ? 'isActive' : ''} key={label}><span>{index + 1}</span>{label}</li>)}</ol><section className="signupCard" aria-live="polite">
    ...
    {step === 4 && <><p className="accountEyebrow">4단계</p><h1>가입 정보를 확인해 주세요</h1><dl className="confirmationList"><div><dt>아이디</dt><dd>{form.loginId}</dd></div><div><dt>이름</dt><dd>{form.name}</dd></div><div><dt>휴대전화</dt><dd>{form.phone}</dd></div><div><dt>본인인증 상태</dt><dd>{verificationStatus === 'TEST_READY' ? '테스트 본인인증 완료' : '본인인증 완료'}</dd></div><div><dt>선택 동의</dt><dd>{documents.filter(item => item.requirementCode === 'OPTIONAL' && agreed[item.versionId]).map(item => item.title).join(', ') || '없음'}</dd></div></dl></>}
...
export function CustomerProviderBlocksPage() {
  ...
  useEffect(() => { void load() }, [])
  const release = async (item: ProviderBlock) => { if (!window.confirm(`${item.providerName} 전문가 차단을 해제할까요? 이후 새 요청에서 다시 매칭될 수 있습니다.`)) return; try { setBusy(item.id); await customerAccountApi.releaseProviderBlock(item.id, item.rowVersion); await load() } catch (reason) { setError(message(reason)) } finally { setBusy('') } }
...
export function CustomerNotificationSettingsPage() {
  ...
  const labels = [['webEnabled', 'WEB 내부 알림', '현재 사용 가능'], ['kakaoEnabled', '카카오 알림톡', '업체 계약 후 발송'], ['smsEnabled', 'SMS 문자', '업체 계약 후 발송'], ['emailEnabled', '이메일', '업체 계약 후 발송'], ['pushEnabled', 'Push 알림', '앱 연동 후 발송']] as const
  const enableBrowserPush=async()=>{if(typeof Notification==='undefined')return setError('이 브라우저는 Push 알림을 지원하지 않습니다.');const result=await Notification.requestPermission();setPushPermission(result);setNotice(result==='granted'?'브라우저 알림 표시 권한을 허용했습니다. NHN Push 채널 활성화 후 선택한 알림을 받을 수 있습니다.':result==='denied'?'브라우저 설정에서 알림 권한을 다시 허용할 수 있습니다.':'알림 권한 요청을 닫았습니다.')}
  return <MySoodalLayout title="알림 설정" description="업무 필수 알림과 선택 마케팅 수신을 채널별로 관리합니다."><div className="notificationPreferences">{items.map(item => <article key={item.eventGroupCode}><header><div><strong>{item.eventGroupCode === 'BUSINESS' ? '업무 알림' : '마케팅 알림'}</strong><span>{item.eventGroupCode === 'BUSINESS' ? '요청·견적·계약·일정·결제·환불·A/S·분쟁에 필요한 안내입니다. 서비스 수행에 꼭 필요한 WEB 알림은 설정과 관계없이 제공될 수 있습니다.' : '약관·동의에서 마케팅 수신에 동의한 경우에만 선택 채널로 발송합니다. 언제든 철회할 수 있습니다.'}</span></div></header><div className="notificationChannelChoices">{labels.map(([key, label, status]) => <label key={key}><input type="checkbox" checked={item[key]} onChange={() => void save(item, key)} /><span><strong>{label}</strong><small>{status}</small></span></label>)}</div>{item.eventGroupCode === 'MARKETING' && <label className="notificationNightConsent"><input type="checkbox" checked={item.nightMarketingEnabled} onChange={() => void save(item, 'nightMarketingEnabled')} /><span><strong>야간 마케팅 수신 동의</strong><small>오후 9시~다음 날 오전 8시 발송에 대한 별도 선택 동의입니다.</small></span></label>}{item.eventGroupCode === 'MARKETING' && <p className="notificationConsentState">{item.consentedAt ? `채널 설정 동의 ${date(item.consentedAt)}${item.consentVersion ? ` · 정책 ${item.consentVersion}` : ''}` : '현재 저장된 채널 설정 동의가 없습니다.'}</p>}</article>)}</div><section className="notIntegratedNotice"><strong>브라우저 Push 권한 · {pushPermission}</strong><span>Push 권한은 자동으로 요구하지 않습니다. 아래 버튼을 직접 누른 경우에만 브라우저가 권한을 요청합니다.</span>{pushPermission!=='granted'&&pushPermission!=='unsupported'&&<button type="button" className="accountSecondary" onClick={()=>void enableBrowserPush()}>Push 알림 사용하기</button>}</section><p className="notIntegratedNotice">카카오 알림톡·SMS·이메일·Push 선택값은 미리 저장할 수 있지만, 인증키 등록·관리자 채널 활성화 전에는 외부 발송되지 않습니다.</p><button className="accountSecondary" onClick={() => navigate('/customer/consents')}>마케팅 약관 동의 확인·철회</button>{notice && <p className="successText">{notice}</p>}{error && <p className="accountError">{error}</p>}</MySoodalLayout>
}
...
export function CustomerNotificationCenterPage({ audience = 'customer' }: { audience?: 'customer' | 'provider' }) {
  ...
  const move = async (item: CustomerNotification) => { try { const target = audience === 'provider' ? safeProviderNotificationTarget(item) : await safeNotificationTarget(item); if (target) navigate(target); else setError('연결할 수 있는 안전한 업무 화면이 없습니다.') } catch (reason) { setError(message(reason)) } }
  ...
  if(audience==='provider')return <ProviderAppLayout><section className="mySoodalHeading"><p>SOODAL PARTNERS</p><h1>전문가 알림센터</h1><span>견적·거래·광고·승인·상담 등 전문가 업무 알림을 확인합니다.</span></section><section className="mySoodalContent">{content}</section></ProviderAppLayout>
