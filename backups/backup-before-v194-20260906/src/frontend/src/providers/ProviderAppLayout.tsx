...
const serviceMenu = [['/provider/care', '수달 케어·맞춤 제안'], ['/provider/interior', '수달 인테리어'], ['/provider/emergency', '긴급출동'], ['/provider/after-services', 'A/S·분쟁'], ['/provider/proposals', '제안·공동모집'], ['/provider/advertising', '광고 신청']] as const
const communityMenu = [['/help-room', '수달 도움방'], ['/suggestions', '수달 제안함']] as const
const accountMenu = [['/provider/onboarding', '기본정보·홍보'], ['/provider/services', '서비스 분야'], ['/provider/areas', '활동 지역'], ['/provider/documents', '자격·증빙'], ['/provider/approval', '승인 현황'], ['/provider/blocks', '고객 차단 현황'], ['/provider/exit', '활동 종료']] as const
const mySoodalMenu = accountMenu.slice(0,6)
...
export function ProviderAppLayout({ children, actions }: PropsWithChildren<{ actions?: ReactNode }>) {
  ...
  return <div className="providerAppShell">
    ...
    <header className="providerHeader"><div className="providerHeaderInner"><BrandLogo homePath="/provider" />
      <div className="providerBrandMeta"><span className="providerBrandLabel">수달 파트너스</span><div className="providerBrandApprovalDesktop"><ApprovalStatus approval={approval}/></div>{isFullyApproved?<button className="providerBrandMobileWallet" type="button" onClick={()=>navigate('/provider/wallet')} aria-label={`충전금 ${formatHeaderBalance(walletBalance,walletCurrency)}`}>{formatHeaderBalance(walletBalance,walletCurrency)}</button>:<div className="providerBrandApprovalMobile"><ApprovalStatus approval={approval}/></div>}</div>
      <nav className="providerDesktopNav" aria-label="전문가 주요 메뉴">{desktop.map(([path, label]) => <button className={isActive(pathname, path) ? 'isActive' : ''} key={path} onClick={() => navigate(path)}>{label}</button>)}
        <NavGroup label={`소통${chatUnread+reviewUnread?` (${Math.min(99,chatUnread+reviewUnread)}${chatUnread+reviewUnread>99?'+':''})`:''}`} pathname={pathname} items={communicationMenu} />
        <NavGroup label="서비스" pathname={pathname} items={serviceMenu} />
        <NavGroup label="커뮤니티·지원" pathname={pathname} items={communityMenu} />
        <NavGroup label="마이수달" pathname={pathname} items={accountMenu} />
      </nav>
      <ProviderRequestStatusPopup />
      <div className="providerHeaderActions"><button className="providerTrustBadge" type="button" onClick={() => navigate('/provider/trust')} aria-label={`나의 신뢰도 페이지로 이동, ${formatHeaderTrust(trust)}`}><span>신뢰도</span><strong>{formatHeaderTrust(trust)}</strong></button><button className="providerWalletBalance" type="button" onClick={() => navigate('/provider/wallet')} aria-label={`마이 수달 충전금으로 이동, 현재 잔액 ${formatHeaderBalance(walletBalance, walletCurrency)}`}><span>충전금</span><strong>{formatHeaderBalance(walletBalance, walletCurrency)}</strong></button><button className="providerBell" onClick={() => navigate('/provider/notifications')} aria-label={`읽지 않은 알림 ${unread}개`}>알림{unread > 0 && <span>{unread > 99 ? '99+' : unread}</span>}</button><FriendShareButton className="headerFriendShareButton" title="수달 라이프 전문가 등록" text="수달 라이프에서 전문가로 활동해 보세요." url="https://partner.soodallife.kr/provider/signup" /><button className="providerCustomerHomeButton" type="button" title={`로그인 계정: ${user?.loginId ?? ''}`} onClick={goCustomerHome}>고객 홈</button><button className="providerLogoutButton" type="button" title={`로그인 계정: ${user?.loginId ?? ''}`} onClick={() => void signOut()}>로그아웃</button><button className="providerMenuToggle" type="button" aria-expanded={mobileMenuOpen} aria-controls="provider-mobile-menu" aria-label={mobileMenuOpen ? '전체 메뉴 닫기' : '전체 메뉴 열기'} onClick={() => setMobileMenuOpen(value => !value)}>☰<span>전체 메뉴</span></button></div>
    </div></header>
    {mobileMenuOpen && <><button className="providerMenuBackdrop" type="button" aria-label="전체 메뉴 닫기" onClick={() => setMobileMenuOpen(false)} /><aside id="provider-mobile-menu" className="providerMobileMenu" aria-label="전문가 전체 메뉴"><header><strong>수달 파트너스 전체 메뉴</strong><button type="button" aria-label="전체 메뉴 닫기" onClick={() => setMobileMenuOpen(false)}>×</button></header><MenuSection title="업무" items={desktop} pathname={pathname} onSelect={goFromMenu} /><MenuSection title="소통" items={communicationMenu} pathname={pathname} onSelect={goFromMenu} /><MenuSection title="서비스" items={serviceMenu} pathname={pathname} onSelect={goFromMenu} /><MenuSection title="커뮤니티·지원" items={communityMenu} pathname={pathname} onSelect={goFromMenu} /><MenuSection title="마이수달" items={accountMenu} pathname={pathname} onSelect={goFromMenu} /><section><h2>추천·계정</h2><div className="providerMobileMenuGrid"><button type="button" onClick={() => goFromMenu('/provider/notifications')}>알림 {unread > 0 ? `${unread}개` : ''}</button><button type="button" onClick={() => goFromMenu('/provider/trust')}>신뢰도</button><button type="button" onClick={() => goFromMenu('/provider/wallet')}>충전금</button><button type="button" onClick={goCustomerHome}>고객 홈</button><FriendShareButton title="수달 라이프 전문가 등록" text="수달 라이프에서 전문가로 활동해 보세요." url="https://partner.soodallife.kr/provider/signup" /><button type="button" onClick={() => void signOut()}>로그아웃</button></div></section></aside></>}
    {isMySoodal && <nav className="providerMySoodalNav" aria-label="마이수달 설정 메뉴">{mySoodalMenu.map(([path,label])=><button type="button" className={isActive(pathname,path)?'isActive':''} key={path} onClick={()=>navigate(path)}>{label}</button>)}</nav>}
    ...
    <nav className="providerBottomNav" aria-label="모바일 전문가 메뉴">{mobile.map(([path, label, icon]) => <button key={path} className={isActive(pathname, path) ? 'isActive' : ''} aria-current={isActive(pathname,path)?'page':undefined} onClick={() => navigate(path)}><MobileNavIcon name={icon}/><span className="providerBottomNavLabel">{label}</span>{label === '소통' && chatUnread + reviewUnread > 0 && <b className="providerNavBadge">{chatUnread + reviewUnread > 99 ? '99+' : chatUnread + reviewUnread}</b>}</button>)}</nav>
  </div>
}
...
function ApprovalStatus({approval}:{approval:{approvalStatus:string;activityStatus:string;nextActions:string[];rejectionReason:string|null}|null}) {
  ...
  const restrictions=code==='APPROVED'&&active?['승인 상태입니다. 서비스별 승인·활동지역·증빙·충전금 조건에 따라 견적을 제출할 수 있습니다.']:['고객 요청 매칭과 견적 제출·채택이 제한됩니다.','긴급출동 응답과 인테리어 전문가 참여가 제한됩니다.','신뢰도 최종 점수 산정은 전체 승인 후 진행됩니다.']
...
function NavGroup({ label, pathname, items }: { label: string; pathname: string; items: ReadonlyArray<readonly [string, string]> }) {
  ...
  return <details className={`providerNavGroup${active ? ' isActive' : ''}`}>
    ...
    <div><header><strong>{label}</strong><button type="button" aria-label={`${label} 메뉴 닫기`} onClick={closeProviderHeaderMenus}>×</button></header>{items.map(([path, itemLabel]) => <button type="button" className={isActive(pathname, path) ? 'isActive' : ''} key={path} onClick={() => navigate(path)}>{itemLabel}</button>)}</div>
  </details>
}
