...
if(-not $PSCmdlet.ShouldProcess("API, customer and partner apps at $AppsRoot plus database $DatabaseName",'Deploy V120 provider advertising consultation')){return}
Import-Module WebAdministration
$releaseRoot=$PSScriptRoot;$stamp=Get-Date -Format 'yyyyMMdd-HHmmss'
$backupRoot=Join-Path $BackupParent "backup-before-provider-advertising-consultation-v120-$stamp"
$iisBackupName="Before_ProviderAdvertisingConsultationV120_$stamp"
$databaseBackupPath=Join-Path $DatabaseBackupRoot "${DatabaseName}_PreProviderAdvertisingConsultationV120_$stamp.bak"
$applySql=Join-Path $releaseRoot 'deployment\Apply-ProviderAdvertisingConsultationV120.sql'
$verifySql=Join-Path $releaseRoot 'deployment\Verify-ProviderAdvertisingConsultationV120.sql'
...
try{
    ...
    if($LASTEXITCODE -ne 0){throw 'V120 database apply failed.'}
...
}catch{$deploymentError=$_;Write-Warning "V120 deployment stopped. Database backup: $databaseBackupPath"}
...
if($LASTEXITCODE -ne 0){throw 'V120 database verification failed.'}
...
$customer=Invoke-WebRequest -Uri 'https://soodallife.kr/customer' -UseBasicParsing -TimeoutSec 30
$partner=Invoke-WebRequest -Uri 'https://partner.soodallife.kr/provider/advertising' -UseBasicParsing -TimeoutSec 30
if($customer.StatusCode -ne 200 -or $partner.StatusCode -ne 200){throw 'Post-deployment page verification failed.'}
[PSCustomObject]@{DeploymentSucceeded=$true;IisBackup=$iisBackupName;FileBackup=$backupRoot;DatabaseBackup=$databaseBackupPath;DatabaseChanged=$true;KakaoExternalDeliveryEnabled=$false;Health=$health.status;Readiness=$ready.status;CustomerSite='ok';PartnerAdvertisingPage='ok'}|Format-List
Write-Host 'V120 provider advertising consultation deployment completed.' -ForegroundColor Green
Write-Host 'KakaoTalk delivery remains disabled until the external vendor contract and credentials are configured.' -ForegroundColor Yellow
