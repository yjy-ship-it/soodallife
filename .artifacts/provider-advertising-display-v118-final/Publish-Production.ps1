...
if(-not $PSCmdlet.ShouldProcess("Partner IIS app at $AppsRoot","Deploy V118 provider advertising display")){return}
Import-Module WebAdministration
$releaseRoot=$PSScriptRoot;$source=Join-Path $releaseRoot 'frontend\partner';$target=Join-Path $AppsRoot 'partner';$stamp=Get-Date -Format 'yyyyMMdd-HHmmss';$backupRoot=Join-Path $BackupParent "backup-before-provider-advertising-display-v118-$stamp";$iisBackupName="Before_ProviderAdvertisingDisplayV118_$stamp"
...
Write-Host 'V118 provider advertising display deployment completed.' -ForegroundColor Green
