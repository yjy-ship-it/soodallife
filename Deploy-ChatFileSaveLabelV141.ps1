...
param(
    [string]$ZipPath='D:\SOODALLIFE\SoodalLife-ChatFileSaveLabel-20260825-v141.zip',
    [string]$ReleaseBasePath='D:\SOODALLIFE\releases',
    [switch]$ConfirmProductionDeployment
)
$ErrorActionPreference='Stop'
$expectedSha256='07919D30958D3A7157DE72D83B030EFC94213A4B443D469F965373311908AC6B'
...
$extractPath=Join-Path $ReleaseBasePath "chat-file-save-label-v141-$stamp"
...
$required=@((Join-Path $release 'frontend\customer\index.html'),(Join-Path $release 'frontend\partner\index.html'))
$missing=@($required|Where-Object{-not(Test-Path -LiteralPath $_ -PathType Leaf)})
if($missing.Count){throw "Required deployment files are missing: $($missing -join ', ')"}
Write-Host "2/4 Extracted V141 deployment validated: $release"
Write-Host '3/4 Running protected customer/partner frontend backup and deployment.'
...
Write-Host '4/4 V141 chat file save label deployment completed.' -ForegroundColor Green
