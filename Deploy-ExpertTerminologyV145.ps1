...
param(
    [string]$ZipPath='D:\SOODALLIFE\SoodalLife-ExpertTerminology-20260827-v145.zip',
    [string]$ReleaseBasePath='D:\SOODALLIFE\releases',
    [switch]$ConfirmProductionDeployment
)
$ErrorActionPreference='Stop'
$expectedSha256='E6AA22FDC6F1575AC1825C8F1597E168E7D8E7331B79551AC3520C5113F66DD3'
...
$extractPath=Join-Path $ReleaseBasePath "expert-terminology-v145-$stamp"
...
$required=@(
    ...
    (Join-Path $release 'deployment\Apply-ExpertTerminologyV145.sql'),
    (Join-Path $release 'deployment\Verify-ExpertTerminologyV145.sql')
)
$missing=@($required|Where-Object{-not(Test-Path -LiteralPath $_ -PathType Leaf)})
if($missing.Count){throw "Required deployment files are missing: $($missing -join ', ')"}
Write-Host "2/4 Extracted V145 deployment validated: $release"
Write-Host '3/4 Running protected database, API, customer, partner and admin deployment.'
...
Write-Host '4/4 V145 expert display terminology deployment completed.' -ForegroundColor Green
