...
param([string]$ZipPath='D:\SOODALLIFE\SoodalLife-GeneralSiteVisit-20260824-v132.zip',[string]$ReleaseBasePath='D:\SOODALLIFE\releases',[switch]$ConfirmProductionDeployment)
$ErrorActionPreference='Stop'
$expectedSha256='1EC2FB36BAB07627BDFAFE8E56E18D750E4335762A532965E96572BBF196305A'
...
$stamp=Get-Date -Format 'yyyyMMdd-HHmmss';$extractPath=Join-Path $ReleaseBasePath "general-site-visit-v132-$stamp";New-Item -ItemType Directory -Path $extractPath|Out-Null
...
$required=@((Join-Path $release 'api\SoodalLife.Api.dll'),(Join-Path $release 'api\web.config'),(Join-Path $release 'frontend\customer\index.html'),(Join-Path $release 'frontend\partner\index.html'),(Join-Path $release 'deployment\Apply-GeneralSiteVisitV132.sql'),(Join-Path $release 'deployment\Verify-GeneralSiteVisitV132.sql'))
...
Write-Host "2/4 Extracted V132 deployment validated: $release";Write-Host '3/4 Running protected database, API, customer and partner backup followed by IIS deployment.'
...
Write-Host '4/4 V132 general site visit workflow deployment completed.' -ForegroundColor Green;Write-Host "Extracted release path: $release"
