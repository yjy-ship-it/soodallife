...
param(
    [string]$ZipPath='D:\SOODALLIFE\SoodalLife-ProviderAdvertisingConsultation-20260820-v120.zip',
    [string]$ReleaseBasePath='D:\SOODALLIFE\releases',
    [switch]$ConfirmProductionDeployment
)
$ErrorActionPreference='Stop'
$expectedSha256='1C6BA18ECCCE51A96E886D6019EDD15D8820D8591D58E2814EC5901D8810EA91'
...
$extractPath=Join-Path $ReleaseBasePath "provider-advertising-consultation-v120-$stamp"
...
$required=@(
    ...
    (Join-Path $release 'deployment\Apply-ProviderAdvertisingConsultationV120.sql'),
    (Join-Path $release 'deployment\Verify-ProviderAdvertisingConsultationV120.sql')
)
$missing=@($required|Where-Object{-not(Test-Path -LiteralPath $_ -PathType Leaf)})
if($missing.Count){throw "Required deployment files are missing: $($missing -join ', ')"}
Write-Host "2/4 Extracted V120 deployment validated: $release"
Write-Host '3/4 Running protected DB, file and IIS backup followed by deployment.'
...
Write-Host '4/4 V120 provider advertising consultation deployment completed.' -ForegroundColor Green
