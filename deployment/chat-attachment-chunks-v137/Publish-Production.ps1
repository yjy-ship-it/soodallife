...
if(-not $PSCmdlet.ShouldProcess("API, customer and partner apps at $AppsRoot",'Deploy V137 chunked chat attachments')){return}
Import-Module WebAdministration
$releaseRoot=$PSScriptRoot
$stamp=Get-Date -Format 'yyyyMMdd-HHmmss'
$backupRoot=Join-Path $BackupParent "backup-before-chat-attachment-v137-$stamp"
$iisBackupName="Before_ChatAttachmentV137_$stamp"
...
try{
    ...
    $probe=Join-Path $privateRoot ".v137-write-probe-$stamp.tmp"
    [IO.File]::WriteAllText($probe,'V137_OK',[Text.Encoding]::UTF8)
...
Write-Host 'V137 chunked chat attachment deployment completed.' -ForegroundColor Green
