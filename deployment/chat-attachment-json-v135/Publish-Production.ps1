...
if(-not $PSCmdlet.ShouldProcess("API, customer and partner apps at $AppsRoot",'Deploy V135 JSON chat attachments')){return}
Import-Module WebAdministration
$releaseRoot=$PSScriptRoot
$stamp=Get-Date -Format 'yyyyMMdd-HHmmss'
$backupRoot=Join-Path $BackupParent "backup-before-chat-attachment-v135-$stamp"
$iisBackupName="Before_ChatAttachmentV135_$stamp"
...
try{
    ...
    $probe=Join-Path $privateRoot ".v135-write-probe-$stamp.tmp"
    [IO.File]::WriteAllText($probe,'V135_OK',[Text.Encoding]::UTF8)
...
Write-Host 'V135 JSON chat attachment deployment completed.' -ForegroundColor Green
