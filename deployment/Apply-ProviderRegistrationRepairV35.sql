...
BEGIN TRY
    ...
    DECLARE @RepairMigrationId nvarchar(150) = N'20260816144500_RepairProviderRegistrationLegalDocuments';
    DECLARE @EffectiveFrom datetime2(7) = '2026-08-16T00:00:00';

    IF NOT EXISTS (SELECT 1 FROM dbo.__EFMigrationsHistory WHERE MigrationId = @RepairMigrationId)
    BEGIN
        DECLARE @Seed TABLE
        (
        ...
        DECLARE seed_cursor CURSOR LOCAL FAST_FORWARD FOR
            SELECT Code, DocumentPublicId, VersionPublicId, RequirementCode, DisplayOrder, Title, Content
            FROM @Seed ORDER BY DisplayOrder;
        OPEN seed_cursor;
        FETCH NEXT FROM seed_cursor INTO @Code,@DocumentPublicId,@VersionPublicId,@RequirementCode,@DisplayOrder,@Title,@Content;
        WHILE @@FETCH_STATUS = 0
        BEGIN
            SET @DocumentId = NULL;
            SELECT @DocumentId = id FROM dbo.legal_documents WHERE code = @Code;

            IF @DocumentId IS NULL
            BEGIN
                INSERT INTO dbo.legal_documents(public_id,code,audience_code,requirement_code,display_order,is_active,is_placeholder,created_at,updated_at)
                VALUES(@DocumentPublicId,@Code,'PROVIDER',@RequirementCode,@DisplayOrder,1,0,SYSUTCDATETIME(),SYSUTCDATETIME());
                SET @DocumentId = SCOPE_IDENTITY();
            END
            ELSE
                UPDATE dbo.legal_documents
                SET audience_code='PROVIDER', requirement_code=@RequirementCode, display_order=@DisplayOrder,
                    is_active=1, is_placeholder=0, updated_at=SYSUTCDATETIME()
                WHERE id=@DocumentId;

            IF EXISTS(SELECT 1 FROM dbo.legal_document_versions WHERE public_id=@VersionPublicId)
                UPDATE dbo.legal_document_versions
                SET legal_document_id=@DocumentId, title=@Title, content=@Content, effective_from=@EffectiveFrom,
                    is_active=1, is_placeholder=0
                WHERE public_id=@VersionPublicId;
            ELSE IF NOT EXISTS(
                SELECT 1 FROM dbo.legal_document_versions
                WHERE legal_document_id=@DocumentId AND version_no=1
            )
                INSERT INTO dbo.legal_document_versions(public_id,legal_document_id,version_no,title,content,effective_from,is_active,is_placeholder,created_at)
        ...
        IF (SELECT COUNT(*) FROM dbo.legal_documents WHERE code IN
            ('PROVIDER_TERMS','PROVIDER_PRIVACY_CONSENT','PROVIDER_MARKETING_CONSENT')
            AND audience_code='PROVIDER' AND is_active=1 AND is_placeholder=0) <> 3
            THROW 51000, 'Provider registration legal document repair verification failed.', 1;

        INSERT INTO dbo.__EFMigrationsHistory(MigrationId, ProductVersion)
        VALUES(@RepairMigrationId, N'10.0.10');
    END
