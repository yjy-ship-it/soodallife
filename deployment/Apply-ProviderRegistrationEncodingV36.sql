...
BEGIN TRY
    ...
    DECLARE @MigrationId nvarchar(150) = N'20260816150000_RepairProviderRegistrationLegalDocumentEncoding';
    ...
    BEGIN
        UPDATE v SET
            ...
            v.is_active = 1,
            v.is_placeholder = 0
        FROM dbo.legal_document_versions v
        INNER JOIN dbo.legal_documents d ON d.id = v.legal_document_id
        INNER JOIN (VALUES
    ...
    IF (SELECT COUNT(*)
        ...
        WHERE (d.code = 'PROVIDER_TERMS' AND v.title = N'수달 라이프 공급자 이용약관')
           ...
           OR (d.code = 'PROVIDER_MARKETING_CONSENT' AND v.title = N'공급자 마케팅 정보 수신 동의')) <> 3
        THROW 51000, 'Provider legal document Unicode verification failed.', 1;
