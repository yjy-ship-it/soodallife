...
$backupRoot=Join-Path $BackupParent "backup-before-custom-care-request-v169-$stamp"
$iisBackupName="Before_CustomCareRequestV169_$stamp"
$sources=[ordered]@{
...
foreach($marker in @('desiredMonthlyAmount','desiredVisitAmount','priceNegotiable')){
    if(-not $sourceText.Contains($marker)){throw "V169 customer frontend marker missing: $marker"}
}
...
foreach($marker in @('desiredMonthlyAmount','desiredVisitAmount','priceNegotiable')){
    if(-not $partnerText.Contains($marker)){throw "V169 partner frontend marker missing: $marker"}
}
...
try{
    ...
    $deploymentError=$_
    Write-Warning "V169 deployment stopped. File backup: $backupRoot"
...
foreach($marker in @('desiredMonthlyAmount','desiredVisitAmount','priceNegotiable')){
    if(-not $deployedText.Contains($marker)){throw "V169 deployed customer marker missing: $marker"}
}
...
[PSCustomObject]@{
    ...
    CustomCareRequest='customer-custom-only'
    CustomerPricePreference='structured'
    ProviderCustomProposal='enabled'
    LegacyStandardProducts='preserved-admin-only'
    CustomerApp='ok'
    PartnerApp='ok'
    AdminApp='ok'
}|Format-List
Write-Host 'V169 custom care request workflow deployment completed.' -ForegroundColor Green
Write-Host 'New customer care requests are custom-only. Provider proposals include scope, schedule and monthly or per-visit amounts.' -ForegroundColor Yellow
