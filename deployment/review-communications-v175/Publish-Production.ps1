...
$backupRoot=Join-Path $BackupParent "backup-before-review-communications-v175-$stamp"
$iisBackupName="Before_ReviewCommunicationsV175_$stamp"
$sources=[ordered]@{
...
foreach($marker in @('/customer/communications','review-conversations/unread-count','soodal-review-conversations-read')){
    if(-not $sourceText.Contains($marker)){throw "V175 frontend marker missing: $marker"}
}
...
foreach($marker in @('/provider/communications','review-conversations/unread-count','soodal-review-conversations-read')){
    if(-not $partnerText.Contains($marker)){throw "V175 partner frontend marker missing: $marker"}
}
...
try{
    ...
    $deploymentError=$_
    Write-Warning "V175 deployment stopped. File backup: $backupRoot"
...
foreach($marker in @('/customer/communications','review-conversations/unread-count','soodal-review-conversations-read')){
    if(-not $deployedText.Contains($marker)){throw "V175 deployed frontend marker missing: $marker"}
}
...
[PSCustomObject]@{
    ...
    CustomerCommunication='hub-chat-and-review-comments'
    ProviderCommunication='hub-chat-and-review-replies'
    ReviewUnread='notification-badge-and-thirty-second-refresh'
    ReviewOrdering='latest-activity-first'
    CommentProtection='duplicate-and-rapid-consecutive-blocked'
    CustomerApp='ok'
    PartnerApp='ok'
    AdminApp='ok'
}|Format-List
Write-Host 'V175 review communications deployment completed.' -ForegroundColor Green
Write-Host 'Customer and provider communication hubs, review reply notifications, unread badges and latest-activity ordering are active.' -ForegroundColor Yellow
