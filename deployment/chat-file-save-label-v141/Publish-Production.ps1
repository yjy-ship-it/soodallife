...
if(-not $PSCmdlet.ShouldProcess("Customer and partner apps at $AppsRoot",'Deploy V141 chat file save label')){return}
Import-Module WebAdministration
$releaseRoot=$PSScriptRoot
$stamp=Get-Date -Format 'yyyyMMdd-HHmmss'
$backupRoot=Join-Path $BackupParent "backup-before-chat-file-save-label-v141-$stamp"
$iisBackupName="Before_ChatFileSaveLabelV141_$stamp"
...
try{
    ...
    $deploymentError=$_
    Write-Warning 'V141 deployment failed. Restoring the pre-deployment frontend backup.'
...
if($customer.StatusCode -ne 200 -or $partner.StatusCode -ne 200){throw 'Post-deployment chat page verification failed.'}
[PSCustomObject]@{DeploymentSucceeded=$true;IisBackup=$iisBackupName;FileBackup=$backupRoot;ApiPreserved=$true;DatabaseChanged=$false;CustomerChat='ok';PartnerChat='ok'}|Format-List
Write-Host 'V141 chat file save label deployment completed.' -ForegroundColor Green
