...
if(-not $PSCmdlet.ShouldProcess("API, customer, partner and admin apps at $AppsRoot",'Deploy V158 diversified customer home and mobile footer')){return}
Import-Module WebAdministration
$releaseRoot=$PSScriptRoot;$stamp=Get-Date -Format 'yyyyMMdd-HHmmss'
$backupRoot=Join-Path $BackupParent "backup-before-customer-home-diversified-footer-v158-$stamp"
$iisBackupName="Before_CustomerHomeDiversifiedFooterV158_$stamp"
$databaseBackupPath=Join-Path $DatabaseBackupRoot "${DatabaseName}_PreCustomerHomeDiversifiedFooterV158_$stamp.bak"
...
$customer=Invoke-WebRequest -Uri 'https://soodallife.kr' -UseBasicParsing -TimeoutSec 30;$partner=Invoke-WebRequest -Uri 'https://partner.soodallife.kr' -UseBasicParsing -TimeoutSec 30;$admin=Invoke-WebRequest -Uri 'https://admin.soodallife.kr' -UseBasicParsing -TimeoutSec 30
...
[PSCustomObject]@{DeploymentSucceeded=$true;IisBackup=$iisBackupName;FileBackup=$backupRoot;DatabaseBackup=$databaseBackupPath;AppDataPreserved=$true;AppSettingsPreserved=$true;DatabaseChanged=$false;Health=$health.status;Readiness=$ready.status;CustomerRepresentativeServices='diversified';MobileFooter='collapsed-by-default'}|Format-List
Write-Host 'V158 diversified customer home and mobile footer deployment completed.' -ForegroundColor Green
Write-Host 'Representative services are distributed by major and middle categories. Mobile footer is collapsed by default.' -ForegroundColor Yellow
