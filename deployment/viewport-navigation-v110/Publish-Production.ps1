...
$backupRoot=Join-Path $BackupParent "backup-before-viewport-navigation-v110-$stamp"
$iisBackupName="Before_ViewportNavigationV110_$stamp"
$sources=[ordered]@{
...
foreach($name in $sources.Keys){$index=Join-Path $sources[$name] 'index.html';if(-not(Test-Path -LiteralPath $index -PathType Leaf)){throw "Required frontend is missing: $index"}}
...
Write-Host 'V110 viewport navigation deployment completed.' -ForegroundColor Green
