...
if(-not $PSCmdlet.ShouldProcess("API, customer and partner apps at $AppsRoot",'Deploy V134 reliable chat attachments')){return}
Import-Module WebAdministration
$releaseRoot=$PSScriptRoot
$stamp=Get-Date -Format 'yyyyMMdd-HHmmss'
$backupRoot=Join-Path $BackupParent "backup-before-chat-attachment-v134-$stamp"
$iisBackupName="Before_ChatAttachmentV134_$stamp"
...
try{
    ...
    $probe=Join-Path $privateRoot ".v134-write-probe-$stamp.tmp"
    [IO.File]::WriteAllText($probe,'V134_OK',[Text.Encoding]::UTF8)
...
[PSCustomObject]@{DeploymentSucceeded=$true;IisBackup=$iisBackupName;FileBackup=$backupRoot;AppDataPreserved=$true;DatabaseChanged=$false;PrivateStorageAcl='ok';Health=$health.status;Readiness=$ready.status;CustomerChat='ok';PartnerChat='ok'}|Format-List
Write-Host 'V134 reliable chat attachment deployment completed.' -ForegroundColor Green
