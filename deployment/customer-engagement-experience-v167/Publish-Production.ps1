...
$backupRoot=Join-Path $BackupParent "backup-before-customer-engagement-experience-v167-$stamp"
$iisBackupName="Before_CustomerEngagementExperienceV167_$stamp"
$sources=[ordered]@{
...
foreach($marker in @('headerFriendShareButton','customerSectionLink','my-neighborhood','providerIntroEditor')){
    if(-not $sourceText.Contains($marker)){throw "V167 frontend marker missing: $marker"}
}
...
try{
    ...
    $deploymentError=$_
    Write-Warning "V167 deployment stopped. File backup: $backupRoot"
...
foreach($marker in @('headerFriendShareButton','customerSectionLink','my-neighborhood','providerIntroEditor')){
    if(-not $deployedText.Contains($marker)){throw "V167 deployed frontend marker missing: $marker"}
}
...
[PSCustomObject]@{
    ...
    CustomerExperience='ok'
    ProviderOnboarding='ok'
    EngagementReminders='fail-closed-until-channel-activation'
    CustomerApp='ok'
    PartnerApp='ok'
    AdminApp='ok'
}|Format-List
Write-Host 'V167 customer engagement experience deployment completed.' -ForegroundColor Green
Write-Host 'Database and production settings were not changed. KAKAO reminder templates remain inactive until approved and explicitly activated.' -ForegroundColor Yellow
