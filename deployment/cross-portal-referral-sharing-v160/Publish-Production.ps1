...
if(-not $PSCmdlet.ShouldProcess("customer, partner and admin apps at $AppsRoot",'Deploy V160 cross-portal navigation and friend sharing')){return}
Import-Module WebAdministration
$releaseRoot=$PSScriptRoot;$stamp=Get-Date -Format 'yyyyMMdd-HHmmss'
$backupRoot=Join-Path $BackupParent "backup-before-cross-portal-referral-sharing-v160-$stamp"
$iisBackupName="Before_CrossPortalReferralSharingV160_$stamp"
...
foreach($marker in @('고객 홈','전문가 홈','친구에게 추천하기','친구에게 참여 추천')){if(-not $sourceText.Contains($marker)){throw "V160 frontend marker missing: $marker"}}
...
try{
    ...
    $deploymentError=$_;Write-Warning "V160 deployment stopped. File backup: $backupRoot"
...
foreach($marker in @('고객 홈','전문가 홈','친구에게 추천하기','친구에게 참여 추천')){if(-not $deployedText.Contains($marker)){throw "V160 deployed frontend marker missing: $marker"}}
...
Write-Host 'V160 cross-portal navigation and friend sharing deployment completed.' -ForegroundColor Green
