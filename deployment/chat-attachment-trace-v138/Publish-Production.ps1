...
if(-not $PSCmdlet.ShouldProcess("API, customer and partner apps at $AppsRoot",'Deploy V138 chat attachment root-cause tracing')){return}
Import-Module WebAdministration
$releaseRoot=$PSScriptRoot
$stamp=Get-Date -Format 'yyyyMMdd-HHmmss'
$backupRoot=Join-Path $BackupParent "backup-before-chat-attachment-v138-$stamp"
$iisBackupName="Before_ChatAttachmentV138_$stamp"
...
try{
    ...
    $probe=Join-Path $privateRoot ".v138-write-probe-$stamp.tmp"
    [IO.File]::WriteAllText($probe,'V138_OK',[Text.Encoding]::UTF8)
...
Write-Host 'V138 chat attachment root-cause tracing deployment completed.' -ForegroundColor Green
