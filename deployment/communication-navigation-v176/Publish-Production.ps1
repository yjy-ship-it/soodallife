...
$backupRoot=Join-Path $BackupParent "backup-before-communication-navigation-v176-$stamp"
$iisBackupName="Before_CommunicationNavigationV176_$stamp"
$sources=[ordered]@{
...
foreach($name in $sources.Keys){
    ...
    if(-not$sw.Contains('soodal-life-shell-v4')){throw "V176 service-worker cache marker missing: $name"}
    if(-not$web.Contains('no-store, no-cache, must-revalidate')){throw "V176 no-store marker missing: $name"}
}
...
try{
    ...
    $deploymentError=$_
    Write-Warning "V176 deployment stopped. File backup: $backupRoot"
...
foreach($marker in @('/customer/communications','/customer/messages','communicationCards')){if(-not$deployedText.Contains($marker)){throw "V176 deployed communication marker missing: $marker"}}
...
[PSCustomObject]@{
    ...
    CustomerCommunicationNavigation='direct-hub-and-working-chat-links'
    CommunicationMenu='no-duplicate-desktop-dropdown'
...
Write-Host 'V176 communication navigation reliability deployment completed.' -ForegroundColor Green
Write-Host 'Customer communication navigation uses direct links, closes obsolete dropdowns and keeps chat tabs visible.' -ForegroundColor Yellow
