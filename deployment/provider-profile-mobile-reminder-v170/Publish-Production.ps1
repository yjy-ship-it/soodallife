...
$backupRoot=Join-Path $BackupParent "backup-before-provider-profile-mobile-reminder-v170-$stamp"
$iisBackupName="Before_ProviderProfileMobileReminderV170_$stamp"
$sources=[ordered]@{
...
foreach($marker in @('providerPrivateProfileGrid','providerBusinessNumberField','providerIntroToolbar')){
    if(-not $sourceText.Contains($marker)){throw "V170 frontend marker missing: $marker"}
}
...
foreach($marker in @('providerPrivateProfileGrid','providerBusinessNumberField','providerIntroToolbar')){
    if(-not $partnerText.Contains($marker)){throw "V170 partner frontend marker missing: $marker"}
}
...
try{
    ...
    $deploymentError=$_
    Write-Warning "V170 deployment stopped. File backup: $backupRoot"
...
foreach($marker in @('providerPrivateProfileGrid','providerBusinessNumberField','providerIntroToolbar')){
    if(-not $deployedText.Contains($marker)){throw "V170 deployed frontend marker missing: $marker"}
}
...
[PSCustomObject]@{
    ...
    ProviderProfileMobile='compact-actions-and-aligned-business-validation'
    ProviderPrivateFields='clarified'
    ProviderOnboardingReminder='web-active-kakao-fail-closed'
    HubUrgentAction='nowrap'
    CustomerApp='ok'
    PartnerApp='ok'
    AdminApp='ok'
}|Format-List
Write-Host 'V170 provider profile mobile and onboarding reminder deployment completed.' -ForegroundColor Green
Write-Host 'Provider profile controls are compact on mobile. Incomplete service-area onboarding reminders are queued without duplicate sends.' -ForegroundColor Yellow
