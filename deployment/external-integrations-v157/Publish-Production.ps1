...
if(-not $ConfirmProductionDeployment){throw 'Safety stop: rerun with -ConfirmProductionDeployment after checking the ZIP hash, integration modes and target paths.'}
if(-not $PSCmdlet.ShouldProcess("API, customer, partner and admin apps at $AppsRoot",'Deploy V157 gated external integrations')){return}
Import-Module WebAdministration
$releaseRoot=$PSScriptRoot;$stamp=Get-Date -Format 'yyyyMMdd-HHmmss'
$backupRoot=Join-Path $BackupParent "backup-before-external-integrations-v157-$stamp"
$iisBackupName="Before_ExternalIntegrationsV157_$stamp"
$databaseBackupPath=Join-Path $DatabaseBackupRoot "${DatabaseName}_PreExternalIntegrationsV157_$stamp.bak"
...
$required=@((Join-Path $sources.Api 'SoodalLife.Api.dll'),(Join-Path $sources.Api 'web.config'),(Join-Path $sources.Customer 'index.html'),(Join-Path $sources.Partner 'index.html'),(Join-Path $sources.Admin 'index.html'),(Join-Path $releaseRoot 'RELEASE-MANIFEST.txt'),(Join-Path $releaseRoot 'EXTERNAL-INTEGRATION-SETTINGS.example.json'))
foreach($file in $required){if(-not(Test-Path -LiteralPath $file -PathType Leaf)){throw "Required release file is missing: $file"}}
if(-not(Get-Command sqlcmd.exe -ErrorAction SilentlyContinue)){throw 'sqlcmd.exe is required for the protected pre-deployment database backup.'}
...
try{foreach($pool in $pools){Stop-Pool $pool};Copy-Checked $sources.Api $targets.Api -ProtectApiData;foreach($name in @('Customer','Partner','Admin')){Copy-Checked $sources[$name] $targets[$name] -Mirror};New-Item -ItemType Directory -Force -Path (Join-Path $targets.Api 'App_Data')|Out-Null}catch{$deploymentError=$_;Write-Warning "V157 deployment stopped. Database backup: $databaseBackupPath";foreach($name in $targets.Keys){$backup=Join-Path $backupRoot $name;if(Test-Path $backup){Copy-Checked $backup $targets[$name] -Mirror}}}finally{foreach($pool in $pools){try{Start-Pool $pool}catch{Write-Warning $_.Exception.Message}}}
...
$customer=Invoke-WebRequest -Uri 'https://soodallife.kr/signup' -UseBasicParsing -TimeoutSec 30;$partner=Invoke-WebRequest -Uri 'https://partner.soodallife.kr/provider/wallet' -UseBasicParsing -TimeoutSec 30;$admin=Invoke-WebRequest -Uri 'https://admin.soodallife.kr' -UseBasicParsing -TimeoutSec 30
...
[PSCustomObject]@{DeploymentSucceeded=$true;IisBackup=$iisBackupName;FileBackup=$backupRoot;DatabaseBackup=$databaseBackupPath;AppDataPreserved=$true;AppSettingsPreserved=$true;DatabaseChanged=$false;Health=$health.status;Readiness=$ready.status;NiceGuard='fail-closed';NhnChannels='gated';TossWalletApproval='server-confirmed';PwaPushPermission='opt-in'}|Format-List
Write-Host 'V157 gated external integrations deployment completed.' -ForegroundColor Green
Write-Host 'Production appsettings were preserved. Merge reviewed keys from EXTERNAL-INTEGRATION-SETTINGS.example.json, start in TEST, and activate channels sequentially.' -ForegroundColor Yellow
