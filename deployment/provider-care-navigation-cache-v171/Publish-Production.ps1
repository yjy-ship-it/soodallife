...
$backupRoot=Join-Path $BackupParent "backup-before-provider-care-navigation-cache-v171-$stamp"
$iisBackupName="Before_ProviderCareNavigationCacheV171_$stamp"
$sources=[ordered]@{
...
foreach($name in $sources.Keys){
    ...
    if(-not$sw.Contains('soodal-life-shell-v4')){throw "V171 service-worker cache marker missing: $name"}
    if(-not$web.Contains('no-store, no-cache, must-revalidate')){throw "V171 no-store marker missing: $name"}
}
...
try{
    ...
    $deploymentError=$_
    Write-Warning "V171 deployment stopped. File backup: $backupRoot"
...
foreach($marker in @('providerCareNavigationV171','controllerchange','updateViaCache')){if(-not$deployedText.Contains($marker)){throw "V171 deployed partner marker missing: $marker"}}
...
[PSCustomObject]@{
    ...
    ProviderCareNavigation='customer-requests-and-sent-proposals-visible'
...
Write-Host 'V171 provider care navigation and cache refresh deployment completed.' -ForegroundColor Green
Write-Host 'Provider care request/proposal navigation is explicit and stale frontend shells are refreshed.' -ForegroundColor Yellow
