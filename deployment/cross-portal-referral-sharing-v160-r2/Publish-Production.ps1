...
if(-not $PSCmdlet.ShouldProcess("customer, partner and admin apps at $AppsRoot",'Deploy V160-R2 cross-portal navigation and friend sharing')){return}
Import-Module WebAdministration
$releaseRoot=$PSScriptRoot;$stamp=Get-Date -Format 'yyyyMMdd-HHmmss'
$backupRoot=Join-Path $BackupParent "backup-before-cross-portal-referral-sharing-v160-r2-$stamp"
$iisBackupName="Before_CrossPortalReferralSharingV160R2_$stamp"
...
foreach($marker in @('providerCustomerHomeButton','customerProviderHomeButton','friendShare','proposalShareButton')){if(-not $sourceText.Contains($marker)){throw "V160-R2 frontend marker missing: $marker"}}
...
try{
    ...
    $deploymentError=$_;Write-Warning "V160-R2 deployment stopped. File backup: $backupRoot"
...
$customer=Invoke-WebRequest -Uri 'https://soodallife.kr' -UseBasicParsing -TimeoutSec 30;$partner=Invoke-WebRequest -Uri 'https://partner.soodallife.kr/provider' -UseBasicParsing -TimeoutSec 30;$proposal=Invoke-WebRequest -Uri 'https://soodallife.kr/proposals' -UseBasicParsing -TimeoutSec 30;$admin=Invoke-WebRequest -Uri 'https://admin.soodallife.kr' -UseBasicParsing -TimeoutSec 30
...
foreach($marker in @('providerCustomerHomeButton','customerProviderHomeButton','friendShare','proposalShareButton')){if(-not $deployedText.Contains($marker)){throw "V160-R2 deployed frontend marker missing: $marker"}}
if($health.status-ne 'ok'-or $ready.status-ne 'ready'-or $customer.StatusCode-ne 200-or $partner.StatusCode-ne 200-or $proposal.StatusCode-ne 200-or $admin.StatusCode-ne 200){throw 'Post-deployment verification failed.'}
[PSCustomObject]@{DeploymentSucceeded=$true;IisBackup=$iisBackupName;FileBackup=$backupRoot;DatabaseChanged=$false;ApiChanged=$false;Health=$health.status;Readiness=$ready.status;ProviderCustomerHome='ok';CustomerProviderHome='ok';FriendReferralShare='ok';ProposalParticipationShare='ok'}|Format-List
Write-Host 'V160-R2 cross-portal navigation and friend sharing deployment completed.' -ForegroundColor Green
Write-Host 'Database schema, data, API and production settings were not changed.' -ForegroundColor Yellow
