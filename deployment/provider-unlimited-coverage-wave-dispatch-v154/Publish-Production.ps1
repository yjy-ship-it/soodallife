...
if(-not $PSCmdlet.ShouldProcess("API, customer, partner and admin apps at $AppsRoot",'Deploy V154 unlimited provider coverage and wave dispatch')){return}
Import-Module WebAdministration
$releaseRoot=$PSScriptRoot
$stamp=Get-Date -Format 'yyyyMMdd-HHmmss'
$backupRoot=Join-Path $BackupParent "backup-before-provider-unlimited-coverage-wave-dispatch-v154-$stamp"
$iisBackupName="Before_ProviderUnlimitedCoverageWaveDispatchV154_$stamp"
$databaseBackupPath=Join-Path $DatabaseBackupRoot "${DatabaseName}_PreProviderUnlimitedCoverageWaveDispatchV154_$stamp.bak"
...
try{
    ...
    $deploymentError=$_
    Write-Warning "V154 deployment stopped. Database backup: $databaseBackupPath"
...
$partner=Invoke-WebRequest -Uri 'https://partner.soodallife.kr/provider/areas' -UseBasicParsing -TimeoutSec 30
if($health.status-ne 'ok'-or $ready.status-ne 'ready'-or $partner.StatusCode-ne 200){throw 'Post-deployment verification failed.'}
[PSCustomObject]@{DeploymentSucceeded=$true;IisBackup=$iisBackupName;FileBackup=$backupRoot;DatabaseBackup=$databaseBackupPath;DatabaseChanged=$false;Health=$health.status;Readiness=$ready.status;ProviderCoverage='unlimited';WaveDispatch='10-per-wave';QuoteClosure='ok';EmergencyAvailability='ok';PartnerApp='ok'}|Format-List
Write-Host 'V154 unlimited provider coverage and wave dispatch deployment completed.' -ForegroundColor Green
Write-Host 'Service and ordinary activity-area count limits are removed. Matching is prefiltered and dispatched in trust-ranked waves of 10 every 5 minutes while fewer than five quotes exist.' -ForegroundColor Yellow
