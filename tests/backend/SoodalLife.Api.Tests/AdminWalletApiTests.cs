...
public sealed class AdminWalletApiTests(AuthenticationWebApplicationFactory factory) : IClassFixture<AuthenticationWebApplicationFactory>
{
    ...
    public async Task DevelopmentCharge_IsConfirmedOnce_WithLedgerBalanceAuditAndNoExistingTradeMutation()
    {
        ...
        var request = new { Amount = 100000m, Reason = "개발환경 충전 프로세스 검증", IdempotencyKey = $"charge-{Guid.NewGuid():N}" };
    ...
    public async Task Adjustment_RequiresReasonSupportsBothDirectionsConcurrencyAndDoesNotAffectOtherWallet()
    {
        ...
        var stale = await client.PostAsJsonAsync($"{BasePath}/{target.ProviderId}/adjustments", new { DirectionCode="INCREASE", Amount=1000, Reason="충전금 조정", IdempotencyKey=$"adjust-{Guid.NewGuid():N}", RowVersion="AQ==" }); Assert.Equal(HttpStatusCode.Conflict, stale.StatusCode);
        ...
        var crossProvider = await client.PostAsJsonAsync($"{BasePath}/{other.ProviderId}/adjustments",new {DirectionCode="INCREASE",Amount=50000,Reason="다른 공급자 재사용 시도",IdempotencyKey=increaseKey,other.RowVersion}); Assert.Equal(HttpStatusCode.Conflict,crossProvider.StatusCode);
    ...
    public async Task RefundWorkflow_ValidatesAmountCreatesRefundLedgerAndWithdrawalDecision()
    {
        ...
        var excessive=await client.PostAsJsonAsync($"{BasePath}/{target.ProviderId}/refunds",new{Amount=40001,Reason="잔여 충전금 환불",IdempotencyKey=$"refund-{Guid.NewGuid():N}"});Assert.Equal(HttpStatusCode.Conflict,excessive.StatusCode);
        var created=await (await client.PostAsJsonAsync($"{BasePath}/{target.ProviderId}/refunds",new{Amount=40000,Reason="탈퇴 전 잔여 충전금 환불",IdempotencyKey=$"refund-{Guid.NewGuid():N}"})).Content.ReadFromJsonAsync<AdminRefundRequestResponse>();Assert.Equal("REQUESTED",created!.StatusCode);
    ...
    private async Task<TestWallet> CreateDedicatedWallet(){using var scope=factory.Services.CreateScope();var db=scope.ServiceProvider.GetRequiredService<SoodalLifeDbContext>();var user=new User{LoginId=$"wallet-provider-{Guid.NewGuid():N}",NormalizedLoginId=$"WALLET-{Guid.NewGuid():N}",PasswordHash="테스트 전용 해시",StatusCode="ACTIVE",CreatedAt=DateTime.UtcNow,UpdatedAt=DateTime.UtcNow};db.Users.Add(user);await db.SaveChangesAsync();var provider=new ProviderProfile{UserId=user.Id,BusinessName="수달 테스트 공급자",ApprovalStatusCode="APPROVED",ActivityStatusCode="ACTIVE",CreatedAt=DateTime.UtcNow,UpdatedAt=DateTime.UtcNow};db.ProviderProfiles.Add(provider);await db.SaveChangesAsync();var wallet=new ProviderWallet{ProviderProfileId=provider.Id,CurrencyCode="KRW",StatusCode="ACTIVE",CreatedAt=DateTime.UtcNow,UpdatedAt=DateTime.UtcNow};db.ProviderWallets.Add(wallet);await db.SaveChangesAsync();return new(provider.PublicId,wallet.PublicId,wallet.Id,Convert.ToBase64String(wallet.RowVersion));}
