...
public sealed class NotificationManagementApiTests(AuthenticationWebApplicationFactory factory) : IClassFixture<AuthenticationWebApplicationFactory>
{
    ...
    [Fact]
    public async Task Admin_SearchesPreviewsAndConfiguresChannels_WithoutEnablingUnconfiguredVendors()
    {
        using var admin = Client(); await Login(admin, factory.Credentials[RoleCodes.Admin]);
        var eventType = $"SEARCH_EVENT_{Guid.NewGuid():N}".ToUpperInvariant();
        await CreateTemplate(admin, $"SEARCH-{Guid.NewGuid():N}", eventType, "CUSTOMER", "WEB", "{{customer_name}}님 안내", "업무번호 {{source_no}}를 확인해 주세요.");
        var templates = await admin.GetFromJsonAsync<PagedNotificationResponse<NotificationTemplateResponse>>($"/api/v1/admin/notifications/templates/search?eventType={eventType}&page=1&pageSize=10");
        Assert.Single(templates!.Items);
        var preview = await admin.PostAsJsonAsync("/api/v1/admin/notifications/templates/preview", new { titleTemplate = "{{customer_name}}님", bodyTemplate = "{{source_no}} 확인", variables = new Dictionary<string, string> { ["customer_name"] = "홍길동", ["source_no"] = "REQ-1" } });
        Assert.Equal("홍길동님", (await preview.Content.ReadFromJsonAsync<NotificationTemplatePreviewResponse>())!.Title);
        var channels = await admin.GetFromJsonAsync<List<NotificationChannelSettingResponse>>("/api/v1/admin/notifications/channels");
        Assert.Equal(5, channels!.Count); var kakao = channels.Single(x => x.ChannelCode == "KAKAO"); Assert.False(kakao.AdapterConfigured);
        var enable = await admin.PutAsJsonAsync("/api/v1/admin/notifications/channels/KAKAO", new { isEnabled = true, operationModeCode = "TEST", providerCode = "PENDING_CONTRACT", senderIdentity = (string?)null, replyTo = (string?)null, batchSize = 50, maxAttempts = 5, baseRetrySeconds = 30, maxRetrySeconds = 900, sendWindowStartHour = 0, sendWindowEndHour = 24, kakao.RowVersion });
