...
public sealed class TrustCalculationEngineTests(AuthenticationWebApplicationFactory factory):IClassFixture<AuthenticationWebApplicationFactory>
{
    ...
    public async Task ActiveCalculation_WritesOneEventAndCurrentTogether_AndIsIdempotent()
    {
        ...
        var provider = new ProviderProfile { UserId = user.Id, BusinessName = "신뢰도 산정 검증 공급자", ApprovalStatusCode = "APPROVED", ActivityStatusCode = "ACTIVE" };
        ...
        var liability = new DisputeLiabilityType { Code = "PROVIDER", Name = "공급자 귀책", IsActive = true, DisplayOrder = 1, CreatedAt = now, UpdatedAt = now };
        ...
        await db.SaveChangesAsync();
        db.DisputeResolutions.Add(new DisputeResolution { DisputeCaseId = dispute.Id, LiabilityTypeId = liability.Id, VersionNo = 1, ResultSummary = "공급자 일부 귀책", DecisionDetails = "검증용 구조화 판정", BasisText = "확인 자료", DecidedAt = now, DecidedByUserId = adminId, IsCurrent = true, CreatedAt = now });
        ...
        var rules = TrustPolicyDraftDefaults.RulesJson
            ...
            .Replace("\"liabilityScores\":{}", "\"liabilityScores\":{\"PROVIDER\":80}", StringComparison.Ordinal)
            .Replace("\"sanctionScores\":{}", "\"sanctionScores\":{\"WARNING\":80}", StringComparison.Ordinal);
