...
public sealed class CustomerReviewApiTests(AuthenticationWebApplicationFactory factory):IClassFixture<AuthenticationWebApplicationFactory>
{
    ...
    [Fact] public async Task ReviewOwner_CanUpdateReview_AndIdempotentRetryReturnsSavedContent()
    {var data=await Prepare(true);using var client=Client();await Login(client,RoleCodes.Customer);var created=(await (await Post(client,data)).Content.ReadFromJsonAsync<ReviewResponse>())!;var key=Key();var input=new{BodyText="수정한 서비스 후기입니다.",Ratings=new[]{new{RatingItemId=data.RatingItemId,RatingValue=5m}},FileIds=Array.Empty<Guid>(),IdempotencyKey=key};var first=await client.PutAsJsonAsync($"/api/v1/customers/me/reviews/{created.Id}",input);first.EnsureSuccessStatusCode();var updated=(await first.Content.ReadFromJsonAsync<ReviewResponse>())!;Assert.Equal("수정한 서비스 후기입니다.",updated.BodyText);Assert.Equal(5m,Assert.Single(updated.Ratings).RatingValue);var retry=await client.PutAsJsonAsync($"/api/v1/customers/me/reviews/{created.Id}",input);retry.EnsureSuccessStatusCode();Assert.Equal(updated.BodyText,(await retry.Content.ReadFromJsonAsync<ReviewResponse>())!.BodyText);}

    [Fact] public async Task OtherCustomer_CannotUpdateReview()
    {var data=await Prepare(true);using var owner=Client();await Login(owner,RoleCodes.Customer);var created=(await (await Post(owner,data)).Content.ReadFromJsonAsync<ReviewResponse>())!;using var other=Client();await other.PostAsJsonAsync("/api/v1/auth/login",new{LoginOrEmail=factory.OtherCustomerCredential.LoginId,factory.OtherCustomerCredential.Password});var response=await other.PutAsJsonAsync($"/api/v1/customers/me/reviews/{created.Id}",new{BodyText="권한 없는 수정",Ratings=new[]{new{RatingItemId=data.RatingItemId,RatingValue=4m}},FileIds=Array.Empty<Guid>(),IdempotencyKey=Key()});Assert.Equal(HttpStatusCode.NotFound,response.StatusCode);}

    [Fact] public async Task CreatingReview_ImmediatelyRequestsActiveTrustRecalculation()
    {var data=await Prepare(true);using(var setup=factory.Services.CreateScope()){var db=setup.ServiceProvider.GetRequiredService<SoodalLifeDbContext>();var draft=await db.TrustPolicies.SingleAsync(x=>x.PolicyVersion==TrustPolicyDraftDefaults.Version);draft.StatusCode="ACTIVE";draft.EffectiveFrom=DateTime.UtcNow.AddDays(-1);await db.SaveChangesAsync();}using var client=Client();await Login(client,RoleCodes.Customer);var response=await Post(client,data);response.EnsureSuccessStatusCode();var review=(await response.Content.ReadFromJsonAsync<ReviewResponse>())!;using var verify=factory.Services.CreateScope();var check=verify.ServiceProvider.GetRequiredService<SoodalLifeDbContext>();Assert.True(await check.ProviderTrustCalculationResults.AnyAsync(x=>x.IdempotencyKey==$"trust-review:{review.Id:N}"));var policy=await check.TrustPolicies.SingleAsync(x=>x.PolicyVersion==TrustPolicyDraftDefaults.Version);policy.StatusCode="DRAFT";await check.SaveChangesAsync();}
    ...
    [Fact] public async Task PublicReview_PublishesOnlyCleanSafeAttachment_WithFriendlyMetadataAndProtectedDownload()
    {
        ...
        var download=await publicClient.GetAsync(file.DownloadUrl);download.EnsureSuccessStatusCode();Assert.Equal("image/png",download.Content.Headers.ContentType?.MediaType);Assert.DoesNotContain(originalName,download.Content.Headers.ContentDisposition?.FileName??string.Empty);
    }
    ...
    [Fact] public async Task ReviewOwner_RetainsOriginalAccess_ButOtherCustomerIsDenied()
    {
        ...
        using var owner=Client();await Login(owner,RoleCodes.Customer);var mine=(await owner.GetFromJsonAsync<List<ReviewResponse>>("/api/v1/customers/me/reviews"))!;var file=Assert.Single(Assert.Single(mine,x=>x.Id==value.ReviewId).Files);Assert.Equal(System.IO.Path.ChangeExtension(originalName,".png"),file.FileName);Assert.Equal("OWNER_ORIGINAL",file.PublicationMode);(await owner.GetAsync(file.DownloadUrl)).EnsureSuccessStatusCode();
    ...
    {
        var value=await CreateAttachedReview("01012345678_전문가에게숨김.jpg");await SetSafety(value.FileId,"NOT_INTEGRATED","NOT_INTEGRATED",null);
    ...
    {var data=await Prepare(true);using var customer=Client();await Login(customer,RoleCodes.Customer);var created=(await (await Post(customer,data)).Content.ReadFromJsonAsync<ReviewResponse>())!;using var provider=Client();await Login(provider,RoleCodes.Provider);var response=await provider.PostAsJsonAsync($"/api/v1/providers/me/reviews/{created.Id}/reply",new{BodyText="이용해 주셔서 감사합니다.",IdempotencyKey=Key()});response.EnsureSuccessStatusCode();Assert.Equal("수달 리뷰전문가",(await response.Content.ReadFromJsonAsync<ReviewReplyResponse>())!.ProviderName);}
    ...
    private async Task<AttachedReview> CreateAttachedReview(string fileName)
    {
        ...
        using var content=new MultipartFormDataContent();using var bytes=new ByteArrayContent(System.Convert.FromBase64String("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII="));bytes.Headers.ContentType=new MediaTypeHeaderValue("image/png");content.Add(bytes,"file",System.IO.Path.ChangeExtension(fileName,".png"));
