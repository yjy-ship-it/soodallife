...
public sealed class HelpRoomSuggestionApiTests(AuthenticationWebApplicationFactory factory):IClassFixture<AuthenticationWebApplicationFactory>
{
    ...
    [Fact]
    public async Task SuggestionBox_ProviderReplyNotification_UsesProviderAudience()
    {
        using var provider=Client();await Login(provider,RoleCodes.Provider);
        var created=await provider.PostAsJsonAsync("/api/v1/suggestions",new{typeCode="UX",title="전문가 제안 알림 대상 확인",body="전문가 포털에서 접수한 제안의 답변 알림이 전문가에게 전달되는지 확인합니다.",pageUrl="https://partner.soodallife.kr/suggestions",deviceInfo="test",appVersion="test",idempotencyKey=Guid.NewGuid().ToString("N")});
        Assert.Equal(HttpStatusCode.OK,created.StatusCode);var suggestion=await created.Content.ReadFromJsonAsync<SuggestionResponse>();Assert.NotNull(suggestion);
        using var admin=Client();await Login(admin,RoleCodes.Admin);
        var updated=await admin.PutAsJsonAsync($"/api/v1/admin/suggestions/{suggestion!.Id}",new{statusCode="REVIEWING",assignedAdminId=(Guid?)null,adminReply="전문가 포털에서 답변을 확인할 수 있습니다.",releaseVersion=(string?)null,note="알림 대상 검증",rowVersion=suggestion.RowVersion});
        ...
        var recipient=await (from notification in db.Notifications join value in db.NotificationRecipients on notification.Id equals value.NotificationId where notification.SourcePublicId==suggestion.Id&&notification.TypeCode=="SUGGESTION_ADMIN_REPLIED" select value).SingleAsync();
        Assert.Equal(RoleCodes.Provider,recipient.RecipientRoleCode);
    }
