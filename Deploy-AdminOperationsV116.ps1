...
param(
    [string]$ZipPath='D:\SOODALLIFE\SoodalLife-AdminOperations-20260820-v116.zip',
    [string]$ReleaseBasePath='D:\SOODALLIFE\releases',
    [switch]$ConfirmProductionDeployment
)
$ErrorActionPreference='Stop'
$expectedSha256='9BF944FBBCE918A187F3B21238B73CA21895003AEAE91D21DE1DFE6312712ED2'
...
$extractPath=Join-Path $ReleaseBasePath "admin-operations-v116-$stamp"
...
$required=@(
    ...
    (Join-Path $release 'deployment\Apply-AdminOperationsV116.sql'),(Join-Path $release 'deployment\Verify-AdminOperationsV116.sql')
)
$missing=@($required|Where-Object{-not(Test-Path -LiteralPath $_ -PathType Leaf)})
if($missing.Count){throw "Required deployment files are missing: $($missing -join ', ')"}
Write-Host "2/4 Extracted V116 deployment validated: $release"
Write-Host '3/4 Running protected DB/file/IIS backup and deployment.'
...
Write-Host '4/4 V116 admin operations deployment completed.' -ForegroundColor Green
Write-Host 'Retention remains dry-run. External vendor channels remain unchanged.' -ForegroundColor Yellow
