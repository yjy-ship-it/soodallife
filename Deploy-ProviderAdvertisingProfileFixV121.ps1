...
param(
    [string]$ZipPath='D:\SOODALLIFE\SoodalLife-ProviderAdvertisingProfileFix-20260820-v121.zip',
    [string]$ReleaseBasePath='D:\SOODALLIFE\releases',
    [switch]$ConfirmProductionDeployment
)
$ErrorActionPreference='Stop'
$expectedSha256='3D40D90AE5B7AC0BFA1A8AFD5C7B3DDFD1E6DEFC6964212894758C1F68ED125E'
...
$stamp=Get-Date -Format 'yyyyMMdd-HHmmss';$extractPath=Join-Path $ReleaseBasePath "provider-advertising-profile-fix-v121-$stamp"
...
$required=@((Join-Path $release 'api\SoodalLife.Api.dll'),(Join-Path $release 'api\web.config'),(Join-Path $release 'frontend\customer\index.html'),(Join-Path $release 'frontend\customer\web.config'),(Join-Path $release 'frontend\partner\index.html'),(Join-Path $release 'frontend\partner\web.config'))
$missing=@($required|Where-Object{-not(Test-Path -LiteralPath $_ -PathType Leaf)})
if($missing.Count){throw "Required deployment files are missing: $($missing -join ', ')"}
Write-Host "2/4 Extracted V121 deployment validated: $release"
Write-Host '3/4 Running protected file and IIS backup followed by deployment.'
...
Write-Host '4/4 V121 provider advertising profile fix deployment completed.' -ForegroundColor Green
