...
if(-not $PSCmdlet.ShouldProcess("API, customer, partner and admin apps at $AppsRoot plus database $DatabaseName",'Deploy V155 customer request abuse protection')){return}
Import-Module WebAdministration
$releaseRoot=$PSScriptRoot
$stamp=Get-Date -Format 'yyyyMMdd-HHmmss'
$backupRoot=Join-Path $BackupParent "backup-before-customer-request-abuse-protection-v155-$stamp"
$iisBackupName="Before_CustomerRequestAbuseProtectionV155_$stamp"
$databaseBackupPath=Join-Path $DatabaseBackupRoot "${DatabaseName}_PreCustomerRequestAbuseProtectionV155_$stamp.bak"
$applySql=Join-Path $releaseRoot 'deployment\Apply-CustomerRequestAbuseProtectionV155.sql'
$verifySql=Join-Path $releaseRoot 'deployment\Verify-CustomerRequestAbuseProtectionV155.sql'
$sources=[ordered]@{
...
try{
    ...
    if($LASTEXITCODE-ne 0){throw 'V155 database apply failed.'}
    ...
    $deploymentError=$_
    Write-Warning "V155 deployment stopped. Database backup: $databaseBackupPath"
...
if($LASTEXITCODE-ne 0){throw 'V155 database verification failed.'}
...
$customer=Invoke-WebRequest -Uri 'https://soodallife.kr/customer/requests/new' -UseBasicParsing -TimeoutSec 30
$partner=Invoke-WebRequest -Uri 'https://partner.soodallife.kr/provider/requests' -UseBasicParsing -TimeoutSec 30
...
[PSCustomObject]@{
    ...
    QuoteViewTracking='ok'
    NicePublishVerification='excluded'
}|Format-List
Write-Host 'V155 customer request abuse protection deployment completed.' -ForegroundColor Green
Write-Host 'NICE phone identity verification before request publication is intentionally excluded.' -ForegroundColor Yellow
