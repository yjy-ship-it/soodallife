...
$backupRoot=Join-Path $BackupParent "backup-before-help-room-role-mobile-v168-$stamp"
$iisBackupName="Before_HelpRoomRoleMobileV168_$stamp"
$sources=[ordered]@{
...
foreach($marker in @('providerHelpHero','helpProviderAction','providerHelpGrid','helpForm')){
    if(-not $sourceText.Contains($marker)){throw "V168 frontend marker missing: $marker"}
}
...
try{
    ...
    $deploymentError=$_
    Write-Warning "V168 deployment stopped. File backup: $backupRoot"
...
foreach($marker in @('providerHelpHero','helpProviderAction','providerHelpGrid','helpForm')){
    if(-not $deployedText.Contains($marker)){throw "V168 deployed frontend marker missing: $marker"}
}
...
[PSCustomObject]@{
    ...
    CustomerHelpRoomMobile='contained-inputs'
    ProviderHelpRoom='approved-categories-and-action-states'
    CustomerApp='ok'
    PartnerApp='ok'
    AdminApp='ok'
}|Format-List
Write-Host 'V168 help room role and mobile reliability deployment completed.' -ForegroundColor Green
Write-Host 'Customer mobile help-room inputs are contained. Provider help-room lists only eligible open posts and shows answer states.' -ForegroundColor Yellow
