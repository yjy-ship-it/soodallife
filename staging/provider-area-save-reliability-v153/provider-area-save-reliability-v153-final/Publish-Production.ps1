...
if(-not $PSCmdlet.ShouldProcess("API, customer, partner and admin apps at $AppsRoot",'Deploy V153 provider area save reliability')){return}
Import-Module WebAdministration
$releaseRoot=$PSScriptRoot
$stamp=Get-Date -Format 'yyyyMMdd-HHmmss'
$backupRoot=Join-Path $BackupParent "backup-before-provider-area-save-reliability-v153-$stamp"
$iisBackupName="Before_ProviderAreaSaveReliabilityV153_$stamp"
$databaseBackupPath=Join-Path $DatabaseBackupRoot "${DatabaseName}_PreProviderAreaSaveReliabilityV153_$stamp.bak"
...
try{
    ...
    $deploymentError=$_
    Write-Warning "V153 deployment stopped. Database backup: $databaseBackupPath"
...
[PSCustomObject]@{DeploymentSucceeded=$true;IisBackup=$iisBackupName;FileBackup=$backupRoot;DatabaseBackup=$databaseBackupPath;DatabaseChanged=$false;Health=$health.status;Readiness=$ready.status;ProviderAreaSave='ok';EmergencyAvailability='ok';PartnerApp='ok'}|Format-List
Write-Host 'V153 provider area save reliability deployment completed.' -ForegroundColor Green
Write-Host 'Activity-area saves remain successful independently from automatic rematching, and emergency availability is recalculated from active areas.' -ForegroundColor Yellow
