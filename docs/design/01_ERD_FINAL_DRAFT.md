...
- 고객·전문가·관리자는 별도 로그인 계정이 아니라 `users`에 대한 다중 역할이다. 고객/전문가 업무 속성만 프로필로 분리하며, 관리자는 MVP에서 별도 프로필 없이 역할과 감사로그로 식별한다.
- 전문가 승인 현재값은 `provider_profiles`, 결정 이력은 append-only `provider_approval_events`에 보존한다.
