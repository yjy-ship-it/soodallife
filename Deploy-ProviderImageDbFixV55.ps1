[CmdletBinding()]
param(
    [string]$ZipPath = 'D:\SOODALLIFE\SoodalLife-ProviderImageDbFix-20260818-2e647b5-v55.zip',
...
$expectedSha256 = '1542AAB7DD39DDE1ECCFB6196C3D6532700D8691E51E257B8758FF6FA259CE5B'
...
$extractPath = Join-Path $ReleaseBasePath "provider-image-db-fix-v55-$stamp"
...
$requiredFiles = @(
    ...
    (Join-Path $releasePath 'deployment\Apply-ProviderImagePurposeMigration.sql'),
    (Join-Path $releasePath 'deployment\Verify-ProviderImagePurpose.sql')
)
...
try {
    & $deployScript.FullName -ServerInstance $ServerInstance -DatabaseName $DatabaseName
}
finally {
    Pop-Location
}
...
Write-Host 'Next: hard refresh and verify logo/photo upload, API health and readiness.'
