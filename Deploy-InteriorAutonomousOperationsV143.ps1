...
param(
    [string]$ZipPath='D:\SOODALLIFE\SoodalLife-InteriorAutonomousOperations-20260825-v143.zip',
    [string]$ReleaseBasePath='D:\SOODALLIFE\releases',
    [switch]$ConfirmProductionDeployment
)
$ErrorActionPreference='Stop'
$expectedSha256='9F410C702098BD305FBCD2F8DDB5B5BAA0ACAA2581618D714792EA998281441A'
...
$extractPath=Join-Path $ReleaseBasePath "interior-autonomous-operations-v143-$stamp"
...
$required=@((Join-Path $release 'api\SoodalLife.Api.dll'),(Join-Path $release 'frontend\customer\index.html'),(Join-Path $release 'frontend\partner\index.html'),(Join-Path $release 'frontend\admin\index.html'))
$missing=@($required|Where-Object{-not(Test-Path -LiteralPath $_ -PathType Leaf)})
if($missing.Count){throw "Required deployment files are missing: $($missing -join ', ')"}
Write-Host "2/4 Extracted V143 deployment validated: $release"
...
Write-Host '4/4 V143 interior autonomous operations deployment completed.' -ForegroundColor Green
