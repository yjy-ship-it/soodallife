...
param(
    [string]$ZipPath='D:\SOODALLIFE\SoodalLife-CustomerLiveActivityRestore-20260825-v133-r2.zip',
    [string]$ReleaseBasePath='D:\SOODALLIFE\releases',
    [switch]$ConfirmProductionDeployment
)
$ErrorActionPreference='Stop'
$expectedSha256='7F7BDBD8A14347634774C11BD3C1E6ACAA9DBFD014B27E4E512B7C349F88D114'
...
$extractPath=Join-Path $ReleaseBasePath "customer-live-activity-restore-v133-r2-$stamp"
...
$required=@((Join-Path $release 'api\SoodalLife.Api.dll'),(Join-Path $release 'api\web.config'),(Join-Path $release 'frontend\customer\index.html'))
$missing=@($required|Where-Object{-not(Test-Path -LiteralPath $_ -PathType Leaf)})
if($missing.Count){throw "Required deployment files are missing: $($missing -join ', ')"}
Write-Host "2/4 Extracted V133-R2 deployment validated: $release"
Write-Host '3/4 Running protected API/customer backup and IIS deployment.'
...
Write-Host '4/4 V133-R2 customer live activity SQL query fix deployment completed.' -ForegroundColor Green
