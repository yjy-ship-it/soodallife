...
param(
    [string]$ZipPath='D:\SOODALLIFE\SoodalLife-ProviderAdPreviewModal-20260823-v126-r2.zip',
    [string]$ReleaseBasePath='D:\SOODALLIFE\releases',
    [switch]$ConfirmProductionDeployment
)
$ErrorActionPreference='Stop'
$expectedSha256='14D5D9E8B7D5823B3352C979B172E714E8DC516D525C9F51E5B8CF46F27FB092'
...
$extractPath=Join-Path $ReleaseBasePath "provider-ad-preview-modal-v126-r2-$stamp"
...
$required=@((Join-Path $release 'frontend\partner\index.html'),(Join-Path $release 'frontend\partner\web.config'))
$missing=@($required|Where-Object{-not(Test-Path -LiteralPath $_ -PathType Leaf)})
if($missing.Count){throw "Required deployment files are missing: $($missing -join ', ')"}
Write-Host "2/4 Extracted V126-R2 deployment validated: $release"
Write-Host '3/4 Running protected partner backup and IIS state-aware deployment.'
...
Write-Host '4/4 V126-R2 provider advertising preview modal deployment completed.' -ForegroundColor Green
