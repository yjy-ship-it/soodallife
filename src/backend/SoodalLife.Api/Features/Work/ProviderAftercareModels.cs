...
public sealed record ProviderAfterServiceListItem(Guid Id, string CaseNumber, string Subject, string? RequestTitle, string Status, string DisplayStatus,
    string SourceType, DateTime ReceivedAt, DateTime? ScheduledAt, bool ContactAvailable);
public sealed record ProviderAfterServiceDetail(Guid Id, string CaseNumber, ProviderCaseSource Source, string Subject, string? RequestTitle,
...
public sealed record ProviderAfterServiceEvidenceContentInput(string FileName, string ContentType, string Base64Content, string? Role, string? Description);
