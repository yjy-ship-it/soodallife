...
public sealed class CustomerInteriorController(CustomerInteriorService service):ControllerBase
{
    ...
    [HttpPost("projects/{id:guid}/provider-selection")]public Task<ActionResult<CustomerInteriorProjectDetail>> SelectProvider(Guid id,CustomerSelectInteriorProviderRequest input,CancellationToken token)=>Run(()=>service.SelectProvider(id,input,User,token));
    [HttpPost("projects/{id:guid}/provider-selection/release")]public Task<ActionResult<CustomerInteriorProjectDetail>> ReleaseProvider(Guid id,CustomerReleaseInteriorProviderSelectionRequest input,CancellationToken token)=>Run(()=>service.ReleaseProviderSelection(id,input,User,token));
    ...
    [HttpPost("projects/{id:guid}/contracts/{contractId:guid}/record-review")]public Task<ActionResult<CustomerInteriorProjectDetail>> ReviewContract(Guid id,Guid contractId,ReviewInteriorContractRecordRequest input,CancellationToken token)=>Run(()=>service.ReviewContract(id,contractId,input,User,token));
