...
public sealed class ProviderCareController(ProviderCareService service) : ControllerBase
{
    ...
    [HttpPost("applications/{id:guid}/withdraw")] public Task<ActionResult<ProviderCareApplicationItem>> WithdrawApplication(Guid id, WithdrawSubscriptionApplicationRequest input, CancellationToken token) => Run(() => service.WithdrawApplication(id, input, User, token));
    ...
    [HttpPost("contracts/{id:guid}/terminate")] public Task<ActionResult<ProviderCareContractListItem>> Terminate(Guid id, ProviderSubscriptionContractActionRequest input, CancellationToken token) => Run(() => service.RequestTermination(id, input, User, token));
