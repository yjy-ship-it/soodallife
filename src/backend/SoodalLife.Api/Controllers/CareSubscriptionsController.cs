...
public sealed class AdminSubscriptionAccountingController(SubscriptionBillingService service):ControllerBase
{
    ...
    [HttpPost("payments/{id:guid}/fail")] public Task<ActionResult<SubscriptionPaymentResponse>> FailPayment(Guid id,PaymentStateRequest input,CancellationToken token)=>Run(()=>service.FailPayment(id,input,Actor(),token));
    ...
    [HttpPost("payouts/{id:guid}/bank-transfer-completion")] public Task<ActionResult<SubscriptionPayoutResponse>> CompleteBankTransferPayout(Guid id,CompleteBankTransferPayoutRequest input,CancellationToken token)=>Run(()=>service.CompleteBankTransferPayout(id,input,Actor(),token));
