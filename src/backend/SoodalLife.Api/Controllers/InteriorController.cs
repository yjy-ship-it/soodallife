...
public sealed class AdminInteriorController(InteriorProjectService service):ControllerBase
{
    ...
    [HttpPost("projects/{id:guid}/participants")]public Task<ActionResult<InteriorParticipantResponse>> Participant(Guid id,AddInteriorParticipantRequest input,CancellationToken token)=>Run(()=>service.AddParticipant(id,input,Actor(),token));
    [HttpPost("stages/{id:guid}/assignments")]public Task<ActionResult<Guid>> AssignStage(Guid id,AssignInteriorStageRequest input,CancellationToken token)=>Run(()=>service.AssignStage(id,input,Actor(),token));
