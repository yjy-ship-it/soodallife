...
internal sealed class DisputeCaseConfiguration() : EntityConfiguration<DisputeCase>("dispute_cases")
{
    protected override void ConfigureEntity(EntityTypeBuilder<DisputeCase> b)
    {
        ...
        b.ToTable("dispute_cases",t=>{t.HasCheckConstraint("CK_dispute_cases_source","([transaction_id] IS NOT NULL AND [subscription_visit_schedule_id] IS NULL) OR ([transaction_id] IS NULL AND [subscription_visit_schedule_id] IS NOT NULL)");t.HasCheckConstraint("CK_dispute_cases_status","[status_code] IN ('OPEN','UNDER_REVIEW','WAITING_CUSTOMER','WAITING_PROVIDER','RESOLVED','CLOSED')");});
    }
}
