...
export function TransactionDirectPaymentPanel() {
  ...
  const load = useCallback(async () => {
    ...
    try { const value = await api.getDirectPayment(transactionId); setContext(value); setAmount(value.agreedAmount); setError('') }
  ...
  return <section className="workPanel directPaymentPanel" aria-labelledby="direct-payment-title">
    ...
    <p className="privacyNote">서비스 대금은 고객이 공급자에게 직접 지급합니다. 이 화면은 지급 사실 확인 기록이며 PG·송금·에스크로 기능이 아닙니다.</p>
    ...
    {context && !payment && ['IN_PROGRESS', 'COMPLETION_SUBMITTED', 'REVISION_REQUESTED', 'COMPLETED'].includes(context.transactionStatus) && <form onSubmit={submit}>
      ...
      <label>증빙 이미지(선택)<input type="file" accept="image/jpeg,image/png,image/webp" onChange={e => setEvidence(e.target.files?.[0] ?? null)} /></label>
      <button className="primary" type="submit">지급 사실 등록</button>
    </form>}
