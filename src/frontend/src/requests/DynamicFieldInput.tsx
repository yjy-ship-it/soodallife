...
export type DynamicValue =
  string | number | boolean | string[] | FileMetadata[] | StructuredTextValue;
...
function normalizedFieldName(field: RequestField) {
  return `${field.fieldKey} ${field.label}`
    .toLowerCase()
    .replace(/[^\p{L}\p{N}]/gu, "");
}
...
function isInternalHint(value: string) {
  const normalized = value.replace(/\s/g, "");
  return (
    /형식[·.ㆍ]?길[이기]검증/.test(normalized) ||
    /NOT_INTEGRATED/i.test(value) ||
    /^[A-Z][A-Z0-9_:-]{2,}$/.test(value.trim())
  );
}

function customerFacingFieldHint(field: RequestField) {
  return [field.helperText, field.validationRule]
    .map((value) => value?.trim() ?? "")
    .find((value) => value.length > 0 && !isInternalHint(value));
}
...
export function DynamicFieldInput({
  ...
  const customerHint = customerFacingFieldHint(field);
  let control;
  ...
  switch (field.inputType) {
    ...
    case "PERIOD": {
      const periodOptions =
        field.options.length > 0 ? field.options : defaultPeriodOptions;
      control = (
        <select
          {...common}
    ...
    case "NUMBER":
      control = (
        <input
          {...common}
          type="number"
          min={field.validationRule.includes("0 이상") ? 0 : undefined}
    ...
    case "SELECT":
      control = (
        ...
        >
          <option value="">
            {isVisitFrequencyField(field) ? "없음" : "선택하세요"}
          </option>
  ...
  return (
    <div className="formField dynamicField">
      <label
        htmlFor={
          ["RADIO", "CHECKBOX"].includes(field.inputType) ? undefined : id
        }
      >
        {field.label}
      ...
      {control}
      {customerHint && <small>{customerHint}</small>}
    </div>
  );
}
