...
import "./customerRequestVisuals.css";
import { ServiceThumbnail, serviceNameFromPath, useServiceVisualSettings } from "../serviceVisuals/ServiceVisual";
        ...
        <section className="journeyCard">
          {step === 0 && (
            <>
              ...
              {selectedService && <div className="requestSelectedServiceVisual"><ServiceThumbnail name={selectedService} /><div><small>선택한 하위 서비스</small><strong>{selectedService}</strong></div></div>}
  ...
  return (
    <CustomerAppLayout>
      <div className="requestJourney">
        <header className={`journeyHeader soodalHeroHeading requestQuoteHero${visualSettings.bannersEnabled ? " hasFeatureBanner" : ""}`}>
          {visualSettings.bannersEnabled && <img src="/service-visuals/feature-banners/request-quote.webp" alt="" aria-hidden="true" />}
          <div className="requestQuoteHeroCopy">
            <p className="eyebrow">내 요청</p>
            <h1>요청·견적</h1>
            <span>필요한 서비스를 요청하고 도착한 전문가 견적을 한곳에서 비교해 보세요.</span>
          </div>
          <button
