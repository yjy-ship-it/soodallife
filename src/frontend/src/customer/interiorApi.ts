...
export const interiorApi={
  ...
  selectVisit:(projectId:string,visitId:string,detailAddress:string)=>request<InteriorProjectDetail>(`/api/v1/customers/me/interior/projects/${projectId}/site-visits/${visitId}/selection`,post({detailAddress,idempotencyKey:`customer-interior-visit-${visitId}-${crypto.randomUUID()}`})),
  selectProvider:(projectId:string,quoteRevisionId:string,rowVersion:string)=>request<InteriorProjectDetail>(`/api/v1/customers/me/interior/projects/${projectId}/provider-selection`,post({quoteRevisionId,rowVersion,idempotencyKey:`customer-interior-provider-${projectId}-${crypto.randomUUID()}`})),
  releaseProvider:(projectId:string,cancelProject:boolean)=>request<InteriorProjectDetail>(`/api/v1/customers/me/interior/projects/${projectId}/provider-selection/release`,post({cancelProject,idempotencyKey:`customer-interior-provider-release-${projectId}-${crypto.randomUUID()}`})),
  ...
  reviewContract:(projectId:string,contractId:string,matches:boolean,mismatchReason:string|null)=>request<InteriorProjectDetail>(`/api/v1/customers/me/interior/projects/${projectId}/contracts/${contractId}/record-review`,post({matches,mismatchReason,idempotencyKey:`customer-interior-contract-review-${contractId}-${crypto.randomUUID()}`})),
