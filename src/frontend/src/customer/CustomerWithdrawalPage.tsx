...
const labels: Record<string, string> = { REQUESTED: '신청 접수', UNDER_REVIEW: '관리자 검토 중', BLOCKED_BY_ACTIVE_WORK: '진행 업무 확인 필요', READY_TO_COMPLETE: '최종 확인 가능', COMPLETED: '고객 역할 종료', REJECTED: '신청 거절', CANCELLED: '신청 취소', CUSTOMER: '고객', PROVIDER: '공급자', ADMIN: '관리자' }
...
export function CustomerWithdrawalPage() {
  ...
  const cancel = async () => { if (!data?.request) return; const value = window.prompt('탈퇴 신청 취소 사유를 입력해 주세요.'); if (!value?.trim()) return; setBusy(true); setError(''); try { await customerAccountApi.cancelWithdrawal(data.request.id, value, data.request.rowVersion); await load() } catch (failure) { setError(message(failure)) } finally { setBusy(false) } }
  ...
  return <MySoodalLayout title="계정·회원 탈퇴" description="진행 업무와 기록 보존 원칙을 확인한 뒤 고객 역할 종료를 신청합니다.">
    ...
    <section className="withdrawalPanel"><h2>역할과 기록 안내</h2><ul><li>현재 활성 역할: {readiness?.activeRoles.map(label).join(', ') || '확인 중'}</li><li>고객 역할만 종료하며 공급자 등 다른 역할은 종료하지 않습니다.</li><li>거래·서비스 이력·리뷰·채팅·감사 기록은 즉시 삭제하지 않습니다.</li><li>개인정보 보존·파기와 채팅 보존기간은 정책 확정 전 자동 처리하지 않습니다.</li><li>외부 PG 환불이나 송금을 완료한 것으로 처리하지 않습니다.</li></ul></section>
