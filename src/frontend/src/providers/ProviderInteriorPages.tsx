...
function ProjectDetail({ value, reload }: { value: InteriorDetail; reload: () => Promise<void> }) {
  ...
  return <>{error && <p className="interiorError">{error}</p>}<section className="interiorSummary"><div><small>{value.project.roles.join(' · ')}</small><h2>{value.project.serviceName}</h2><p>{value.project.projectNumber} · {value.project.areaName}</p>{value.project.isFinalProvider&&<mark>고객이 최종 시공 공급자로 선택했습니다.</mark>}</div><strong>{value.project.statusCode}</strong></section><div className="interiorColumns">
