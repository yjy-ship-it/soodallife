...
function Files({ files }: { files: Array<{ id: string; fileName: string; downloadUrl: string }> }) { return <div>{files.map(file => <a className="interiorFile" key={file.id} href={file.downloadUrl}>{file.fileName}</a>)}</div> }
