...
export function ProviderOperationsHomePage() {
  return <AuthenticatedLayout><section className="providerHero"><div><p>SOODAL PARTNERS</p><h1>공급자 업무 홈</h1><span>매칭 요청부터 견적, 일정, 작업완료까지 지금 처리할 실제 업무를 확인합니다.</span></div></section><ProviderOperationsPanel /><section className="providerPanel"><h2>빠른 업무</h2><div className="providerActions"><button className="providerPrimary" onClick={() => navigate('/provider/matched-requests')}>매칭 요청 확인</button><button className="providerSecondary" onClick={() => navigate('/provider/care')}>수달 케어</button><button className="providerSecondary" onClick={() => navigate('/provider/quotes')}>내 견적</button><button className="providerSecondary" onClick={() => navigate('/provider/work')}>거래·작업</button><button className="providerSecondary" onClick={() => navigate('/provider/wallet')}>충전금·수수료</button><button className="providerSecondary" onClick={() => navigate('/provider/onboarding')}>공급자 정보</button></div></section></AuthenticatedLayout>
}
