...
export interface TrustManualAdjustment {providerId:string;scoreBefore:number|null;scoreAfter:number;gradeLabel:string;adjustedAt:string;eventId:string}
export interface TrustRatingItem {id:string;code:string;name:string;description:string|null;minValue:number;maxValue:number;isRequired:boolean;isActive:boolean;displayOrder:number;rowVersion:string}
export interface TrustMasterItem {id:string;code:string;name:string;description:string|null;isActive:boolean;displayOrder:number}
export interface TrustReferenceData {ratingItems:TrustRatingItem[];liabilityTypes:TrustMasterItem[];sanctionTypes:TrustMasterItem[]}
