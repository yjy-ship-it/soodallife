...
import { getAdminCustomer, searchAdminCustomers } from './customerApi'
import type { AdminCustomerDetail, AdminCustomerList } from './customerTypes'

const roleLabel: Record<string, string> = { CUSTOMER: '고객', PROVIDER: '공급자', ADMIN: '관리자' }
...
function CustomerDetail({ customerId }: { customerId: string }) {
  ...
  useEffect(() => { getAdminCustomer(customerId).then(setCustomer).catch((reason: unknown) => setError(reason instanceof Error ? reason.message : '고객 상세를 불러오지 못했습니다.')) }, [customerId])
  ...
  return <>
    ...
    <section className="customerTabPanel">{renderCustomerTab(customer, tab)}</section>
  </>
}

function renderCustomerTab(customer: AdminCustomerDetail, tab: CustomerTab) {
  ...
  if (tab === '요청') return customer.requests.length ? <FlowTable headers={['등록일','서비스·요청','상태','요청지역','견적','채택']} rows={customer.requests.map((item) => [date(item.createdAt), <><strong>{item.serviceName}</strong><small>{item.title}</small></>, label(item.statusCode), <>{item.areaName}<small>{item.detailAddress ?? '상세주소 없음'}</small></>, `${item.quoteCount}건`, item.hasAcceptedQuote ? '채택' : '미채택'])} /> : <Empty>등록된 요청이 없습니다.</Empty>
  if (tab === '견적·채택') return customer.quotes.length ? <FlowTable headers={['제출일','요청','공급자','금액','상태','채택일']} rows={customer.quotes.map((item) => [date(item.submittedAt), item.requestTitle, item.providerName, amount(item.totalAmount, item.currencyCode), label(item.statusCode), item.isAccepted ? date(item.acceptedAt) : '미채택'])} /> : <Empty>받은 견적이 없습니다.</Empty>
  if (tab === '거래') return customer.transactions.length ? <FlowTable headers={['서비스','공급자','상태','견적금액','실제금액','시작·완료']} rows={customer.transactions.map((item) => [item.serviceName, item.providerName, label(item.statusCode), amount(item.agreedAmount, item.currencyCode), amount(item.actualAmount, item.currencyCode), <>{date(item.startedAt)}<small>{date(item.completedAt)}</small></>])} /> : <Empty>거래 이력이 없습니다.</Empty>
  ...
  if (tab === '수리·서비스 이력') return customer.serviceHistory.length ? <div className="customerHistoryList">{customer.serviceHistory.map((item) => <article className="customerHistoryCard" key={item.id}><div><strong>{item.title}</strong><span>{label(item.eventTypeCode)} · {date(item.occurredAt)}</span></div><p>{item.summary}</p><dl><div><dt>서비스</dt><dd>{item.categoryName ?? '기록 없음'}</dd></div><div><dt>공급자</dt><dd>{item.providerName ?? '기록 없음'}</dd></div><div><dt>금액</dt><dd>{amount(item.totalAmount, item.currencyCode ?? 'KRW')}</dd></div><div><dt>보증 종료일</dt><dd>{item.warrantyEndDate ?? '기록 없음'}</dd></div></dl></article>)}</div> : <Empty>수리·서비스 이력이 없습니다.</Empty>
