import type { AdminProviderRequirement, AdminProviderRequirementList, SaveAdminCategoryProviderRequirementInput } from './providerRequirementTypes'
...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) {
    let message = '공급자 요건을 불러오지 못했습니다.'
    try {
