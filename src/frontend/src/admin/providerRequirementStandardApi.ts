import type { AdminProviderDocumentType, AdminProviderRequirementDefinition, AdminProviderRequirementStandards, AdminProviderRequirementType, SaveProviderDocumentTypeInput, SaveProviderRequirementDefinitionInput, SaveProviderRequirementTypeInput } from './providerRequirementStandardTypes'
...
async function request<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) { let message = '공급자 요건 기준정보 요청을 처리하지 못했습니다.'; try { const error = await response.json() as ApiError; if (error.message) message = error.message } catch { /* 공통 안내 사용 */ } throw new Error(message) }
