...
export function CustomerQuotesPanel({ requestId }: { requestId: string }) {
  ...
  const blockProvider = async () => { if (!profile || !window.confirm(`${profile.businessName} 공급자를 차단할까요? 기존 거래와 기록은 유지되며 새로운 매칭만 제한됩니다.`)) return; try { await customerAccountApi.blockProvider(profile.id, null, null); setMessage('공급자를 차단했습니다. 마이수달에서 해제할 수 있습니다.'); setProfile(null); await load() } catch (reason) { setError(reason instanceof Error ? reason.message : '공급자를 차단하지 못했습니다.') } }
  const accept = async () => {
    if (!selected) return
