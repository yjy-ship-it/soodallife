...
param(
    [string]$ZipPath='D:\SOODALLIFE\SoodalLife-CustomerLiveFeedAdvertising-20260825-v144.zip',
    [string]$ReleaseBasePath='D:\SOODALLIFE\releases',
    [switch]$ConfirmProductionDeployment
)
$ErrorActionPreference='Stop'
$expectedSha256='AE24767D32B5FD08E97380701802D91B41F0BE6CFFCC87489A3335FF862E09BB'
if(-not $ConfirmProductionDeployment){throw 'Safety stop: add -ConfirmProductionDeployment only after checking the ZIP hash and backup paths.'}
...
$extractPath=Join-Path $ReleaseBasePath "customer-live-feed-advertising-v144-$stamp"
...
$required=@(
    ...
    (Join-Path $release 'deployment\Apply-CustomerLiveFeedAdvertisingV144.sql'),
    (Join-Path $release 'deployment\Verify-CustomerLiveFeedAdvertisingV144.sql')
)
$missing=@($required|Where-Object{-not(Test-Path -LiteralPath $_ -PathType Leaf)})
if($missing.Count){throw "Required deployment files are missing: $($missing -join ', ')"}
Write-Host "2/4 Extracted V144 deployment validated: $release"
Write-Host '3/4 Running protected database, API, frontend backup and IIS deployment.'
...
Write-Host '4/4 V144 customer live activity feed advertising deployment completed.' -ForegroundColor Green
