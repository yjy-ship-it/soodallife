...
$required=@((Join-Path $sources.API 'SoodalLife.Api.dll'),(Join-Path $sources.API 'web.config'),(Join-Path $sources.API 'V244-R3-API-MARKER.txt'),(Join-Path $sources.Customer 'index.html'),(Join-Path $sources.Partner 'index.html'),(Join-Path $sources.Admin 'index.html'),$applySql,$verifySql,$workflowApplySql,$workflowVerifySql,$districtApplySql,$districtVerifySql,$retireCitiesApplySql,$retireCitiesVerifySql)
...
if(-not $PSCmdlet.ShouldProcess("Database $DatabaseName plus API, customer, partner and admin apps",'Deploy V244-R3 saved-address unlink and parent city deletion')){return}

Import-Module WebAdministration
$stamp=Get-Date -Format 'yyyyMMdd-HHmmss'
$backupRoot=Join-Path $BackupParent "backup-before-administrative-districts-v244-r3-$stamp"
$databaseBackupPath=Join-Path $DatabaseBackupRoot "${DatabaseName}_PreAdministrativeDistrictsV244R3_$stamp.bak"
$iisBackup="Before_AdministrativeDistrictsV244R3_$stamp"
...
$marker=(Get-Content -LiteralPath (Join-Path $targets.API 'V244-R3-API-MARKER.txt') -Raw -Encoding UTF8).Contains('V244-R3 saved-address links cleared; parent city rows deleted; districts preserved: enabled')
