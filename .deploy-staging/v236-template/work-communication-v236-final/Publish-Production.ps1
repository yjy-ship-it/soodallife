...
$required = @(
    ...
    (Join-Path $sources.API 'V236-API-MARKER.txt'),
    (Join-Path $sources.Customer 'index.html'),
    (Join-Path $sources.Partner 'index.html'),
    (Join-Path $sources.Admin 'index.html')
)
foreach ($file in $required) { if (-not (Test-Path -LiteralPath $file -PathType Leaf)) { throw "Required V236 file missing: $file" } }
...
if (-not $PSCmdlet.ShouldProcess("API, customer, partner and admin apps at $resolved", 'Deploy V236 work communication improvements')) { return }

Import-Module WebAdministration
$stamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$backupRoot = Join-Path $BackupParent "backup-before-work-communication-v236-$stamp"
$iisBackup = "Before_WorkCommunicationV236_$stamp"
...
try {
    ...
    $failure = $_
    Write-Warning "V236 deployment stopped. Restoring files from $backupRoot."
...
$marker = (Get-Content -LiteralPath (Join-Path $targets.API 'V236-API-MARKER.txt') -Raw -Encoding UTF8).Contains('Full work quote detail: enabled')
...
Write-Host 'V236 work communication deployment completed.' -ForegroundColor Green
