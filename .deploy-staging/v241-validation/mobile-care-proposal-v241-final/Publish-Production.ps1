...
$applySql=Join-Path $root 'deployment\Apply-MobileCareProposalV241.sql'
$verifySql=Join-Path $root 'deployment\Verify-MobileCareProposalV241.sql'
$required=@((Join-Path $sources.API 'SoodalLife.Api.dll'),(Join-Path $sources.API 'web.config'),(Join-Path $sources.API 'V241-API-MARKER.txt'),(Join-Path $sources.Customer 'index.html'),(Join-Path $sources.Partner 'index.html'),(Join-Path $sources.Admin 'index.html'),$applySql,$verifySql)
foreach($file in $required){if(-not(Test-Path -LiteralPath $file -PathType Leaf)){throw "Required V241 file is missing: $file"}}
...
if(-not $PSCmdlet.ShouldProcess("Database $DatabaseName plus API, customer, partner and admin apps",'Deploy V241 mobile, care and proposal fixes')){return}

Import-Module WebAdministration
$stamp=Get-Date -Format 'yyyyMMdd-HHmmss'
$backupRoot=Join-Path $BackupParent "backup-before-mobile-care-proposal-v241-$stamp"
$databaseBackupPath=Join-Path $DatabaseBackupRoot "${DatabaseName}_PreMobileCareProposalV241_$stamp.bak"
$iisBackup="Before_MobileCareProposalV241_$stamp"
...
try{
    ...
    if($LASTEXITCODE-ne 0){throw 'V241 database apply failed.'}
...
}catch{$failure=$_;Write-Warning "V241 deployment stopped. File backup: $backupRoot. Database backup: $databaseBackupPath"}
...
if($LASTEXITCODE-ne 0){throw 'V241 database verification failed.'}
...
$marker=(Get-Content -LiteralPath (Join-Path $targets.API 'V241-API-MARKER.txt') -Raw -Encoding UTF8).Contains('V241 mobile care proposal fixes: enabled')
...
Write-Host 'V241 mobile, care and proposal deployment completed.' -ForegroundColor Green
