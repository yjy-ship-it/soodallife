...
$applySql=Join-Path $root 'deployment\Apply-MobileCareProposalV242.sql'
$verifySql=Join-Path $root 'deployment\Verify-MobileCareProposalV242.sql'
$workflowApplySql=Join-Path $root 'deployment\Apply-ProviderWorkflowV242.sql'
$workflowVerifySql=Join-Path $root 'deployment\Verify-ProviderWorkflowV242.sql'
$required=@((Join-Path $sources.API 'SoodalLife.Api.dll'),(Join-Path $sources.API 'web.config'),(Join-Path $sources.API 'V242-API-MARKER.txt'),(Join-Path $sources.Customer 'index.html'),(Join-Path $sources.Partner 'index.html'),(Join-Path $sources.Admin 'index.html'),$applySql,$verifySql,$workflowApplySql,$workflowVerifySql)
foreach($file in $required){if(-not(Test-Path -LiteralPath $file -PathType Leaf)){throw "Required V242 file is missing: $file"}}
...
if(-not $PSCmdlet.ShouldProcess("Database $DatabaseName plus API, customer, partner and admin apps",'Deploy V242 provider workflow and catalog fixes')){return}

Import-Module WebAdministration
$stamp=Get-Date -Format 'yyyyMMdd-HHmmss'
$backupRoot=Join-Path $BackupParent "backup-before-provider-workflow-v242-$stamp"
$databaseBackupPath=Join-Path $DatabaseBackupRoot "${DatabaseName}_PreMobileCareProposalV242_$stamp.bak"
$iisBackup="Before_MobileCareProposalV242_$stamp"
...
try{
    ...
    if($LASTEXITCODE-ne 0){throw 'V242 database apply failed.'}
    & sqlcmd.exe -S $ServerInstance -E -C -I -b -f 65001 -d $DatabaseName -i $workflowApplySql
    if($LASTEXITCODE-ne 0){throw 'V242 workflow and catalog data apply failed.'}
...
}catch{$failure=$_;Write-Warning "V242 deployment stopped. File backup: $backupRoot. Database backup: $databaseBackupPath"}
...
if($LASTEXITCODE-ne 0){throw 'V242 database verification failed.'}
& sqlcmd.exe -S $ServerInstance -E -C -I -b -f 65001 -d $DatabaseName -i $workflowVerifySql
if($LASTEXITCODE-ne 0){throw 'V242 workflow and catalog verification failed.'}
...
$marker=(Get-Content -LiteralPath (Join-Path $targets.API 'V242-API-MARKER.txt') -Raw -Encoding UTF8).Contains('V242 provider workflow and catalog fixes: enabled')
