...
if(-not $PSCmdlet.ShouldProcess("API, customer, partner and admin apps at $AppsRoot",'Deploy V143 interior autonomous operations')){return}
Import-Module WebAdministration
$releaseRoot=$PSScriptRoot
$stamp=Get-Date -Format 'yyyyMMdd-HHmmss'
$backupRoot=Join-Path $BackupParent "backup-before-interior-autonomous-operations-v143-$stamp"
$iisBackupName="Before_InteriorAutonomousOperationsV143_$stamp"
...
try{
    ...
    $deploymentError=$_
    Write-Warning 'V143 deployment failed. Restoring the pre-deployment file backup.'
...
$customer=Invoke-WebRequest -Uri 'https://soodallife.kr/customer/interior/projects' -UseBasicParsing -TimeoutSec 30
$partner=Invoke-WebRequest -Uri 'https://partner.soodallife.kr/provider/interior' -UseBasicParsing -TimeoutSec 30
$admin=Invoke-WebRequest -Uri 'https://admin.soodallife.kr/admin/interior' -UseBasicParsing -TimeoutSec 30
if($health.status-ne 'ok'-or $readiness.status-ne 'ready'-or $customer.StatusCode-ne 200-or $partner.StatusCode-ne 200-or $admin.StatusCode-ne 200){throw 'Post-deployment health or interior page verification failed.'}
[PSCustomObject]@{DeploymentSucceeded=$true;IisBackup=$iisBackupName;FileBackup=$backupRoot;AppDataPreserved=$true;AppSettingsPreserved=$true;DatabaseChanged=$false;Health=$health.status;Readiness=$readiness.status;CustomerInterior='ok';PartnerInterior='ok';AdminInterior='ok'}|Format-List
Write-Host 'V143 interior autonomous operations deployment completed.' -ForegroundColor Green
