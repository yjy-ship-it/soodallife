...
$required=@((Join-Path $sources.API 'SoodalLife.Api.dll'),(Join-Path $sources.API 'web.config'),(Join-Path $sources.API 'V243-API-MARKER.txt'),(Join-Path $sources.Customer 'index.html'),(Join-Path $sources.Partner 'index.html'),(Join-Path $sources.Admin 'index.html'),$applySql,$verifySql,$workflowApplySql,$workflowVerifySql,$districtApplySql,$districtVerifySql)
...
if(-not $PSCmdlet.ShouldProcess("Database $DatabaseName plus API, customer, partner and admin apps",'Deploy V243 provider workflow and catalog fixes')){return}

Import-Module WebAdministration
$stamp=Get-Date -Format 'yyyyMMdd-HHmmss'
$backupRoot=Join-Path $BackupParent "backup-before-provider-workflow-v243-$stamp"
$databaseBackupPath=Join-Path $DatabaseBackupRoot "${DatabaseName}_PreMobileCareProposalV243_$stamp.bak"
$iisBackup="Before_MobileCareProposalV243_$stamp"
...
$marker=(Get-Content -LiteralPath (Join-Path $targets.API 'V243-API-MARKER.txt') -Raw -Encoding UTF8).Contains('V243 administrative district catalog expansion: enabled')
