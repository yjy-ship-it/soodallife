...
$required = @(
    ...
    (Join-Path $sources.API 'V238-API-MARKER.txt'),
    (Join-Path $sources.Customer 'index.html'),
    (Join-Path $sources.Partner 'index.html'),
    (Join-Path $sources.Admin 'index.html')
)
foreach ($file in $required) { if (-not (Test-Path -LiteralPath $file -PathType Leaf)) { throw "Required V238 file missing: $file" } }
...
if (-not $PSCmdlet.ShouldProcess("API, customer, partner and admin apps at $resolved", 'Deploy V238 quote summary display improvement')) { return }

Import-Module WebAdministration
$stamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$backupRoot = Join-Path $BackupParent "backup-before-quote-summary-copy-v236-$stamp"
$iisBackup = "Before_QuoteSummaryCopyV238_$stamp"
...
try {
    ...
    $failure = $_
    Write-Warning "V238 deployment stopped. Restoring files from $backupRoot."
...
$marker = (Get-Content -LiteralPath (Join-Path $targets.API 'V238-API-MARKER.txt') -Raw -Encoding UTF8).Contains('Quote placeholder replacement: enabled')
...
[PSCustomObject]@{
    ...
    QuotePlaceholderReplacement = 'enabled'
    QuoteSummaryFallback = 'item-based'
    QuoteTermsFallback = 'customer-agreement-guidance'
...
Write-Host 'V238 quote summary display improvement deployment completed.' -ForegroundColor Green
