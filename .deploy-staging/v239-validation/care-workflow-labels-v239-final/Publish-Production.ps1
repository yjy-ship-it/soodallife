...
$required = @(
    ...
    (Join-Path $sources.API 'V239-API-MARKER.txt'),
    (Join-Path $sources.Customer 'index.html'),
    (Join-Path $sources.Partner 'index.html'),
    (Join-Path $sources.Admin 'index.html')
)
foreach ($file in $required) { if (-not (Test-Path -LiteralPath $file -PathType Leaf)) { throw "Required V239 file missing: $file" } }
...
if (-not $PSCmdlet.ShouldProcess("API, customer, partner and admin apps at $resolved", 'Deploy V239 care workflow terminology and guide')) { return }

Import-Module WebAdministration
$stamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$backupRoot = Join-Path $BackupParent "backup-before-care-workflow-labels-v239-$stamp"
$iisBackup = "Before_CareWorkflowLabelsV239_$stamp"
...
try {
    ...
    $failure = $_
    Write-Warning "V239 deployment stopped. Restoring files from $backupRoot."
...
$marker = (Get-Content -LiteralPath (Join-Path $targets.API 'V239-API-MARKER.txt') -Raw -Encoding UTF8).Contains('Care workflow labels: updated')
...
[PSCustomObject]@{
    ...
    CareWorkflowLabels = 'updated'
    CareProcessGuide = 'customer-to-next-visit'
    ProviderVisitGuide = 'scheduled-to-customer-confirmation'
...
Write-Host 'V239 care workflow terminology and guide deployment completed.' -ForegroundColor Green
