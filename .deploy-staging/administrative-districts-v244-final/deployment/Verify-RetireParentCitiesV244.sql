...
) THROW 52441, 'V244 verification failed: one or more parent city choices are still active.', 1;
...
) THROW 52442, 'V244 verification failed: one or more district choices were removed or assigned incorrectly.', 1;

SELECT N'V244_OK' AS verification,
       (SELECT COUNT(*) FROM @Cities city WHERE EXISTS
        (SELECT 1 FROM dbo.administrative_areas area WHERE area.area_code=city.area_code AND area.area_name=city.area_name AND area.is_active=0)) AS retired_city_count,
