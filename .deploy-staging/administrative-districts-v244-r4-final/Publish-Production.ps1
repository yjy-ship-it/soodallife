...
$required=@((Join-Path $sources.API 'SoodalLife.Api.dll'),(Join-Path $sources.API 'web.config'),(Join-Path $sources.API 'V244-R4-API-MARKER.txt'),(Join-Path $sources.Customer 'index.html'),(Join-Path $sources.Partner 'index.html'),(Join-Path $sources.Admin 'index.html'),$applySql,$verifySql,$workflowApplySql,$workflowVerifySql,$districtApplySql,$districtVerifySql,$retireCitiesApplySql,$retireCitiesVerifySql)
...
if(-not $PSCmdlet.ShouldProcess("Database $DatabaseName plus API, customer, partner and admin apps",'Deploy V244-R4 unreferenced request cleanup and parent city deletion')){return}

Import-Module WebAdministration
$stamp=Get-Date -Format 'yyyyMMdd-HHmmss'
$backupRoot=Join-Path $BackupParent "backup-before-administrative-districts-v244-r4-$stamp"
$databaseBackupPath=Join-Path $DatabaseBackupRoot "${DatabaseName}_PreAdministrativeDistrictsV244R4_$stamp.bak"
$iisBackup="Before_AdministrativeDistrictsV244R4_$stamp"
...
$marker=(Get-Content -LiteralPath (Join-Path $targets.API 'V244-R4-API-MARKER.txt') -Raw -Encoding UTF8).Contains('V244-R4 unreferenced requests removed; parent cities deleted; districts preserved: enabled')
