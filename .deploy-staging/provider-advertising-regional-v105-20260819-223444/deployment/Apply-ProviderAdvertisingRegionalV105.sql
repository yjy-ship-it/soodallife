...
BEGIN TRY
 ...
 BEGIN
  ...
  UPDATE dbo.provider_advertising_applications SET base_fee_amount=fee_amount WHERE base_fee_amount=0;
  UPDATE dbo.provider_advertising_rate_policies SET province_unit_amount=10000,district_unit_amount=2000,regional_fee_cap_amount=30000 WHERE province_unit_amount=0 AND district_unit_amount=0 AND regional_fee_cap_amount=0;
  ...
  ALTER TABLE dbo.provider_advertising_rate_policies ADD CONSTRAINT CK_provider_ad_rate_amount CHECK(fixed_amount>0 AND province_unit_amount>=0 AND district_unit_amount>=0 AND regional_fee_cap_amount>=0);
  ...
  ALTER TABLE dbo.provider_advertising_applications ADD CONSTRAINT CK_provider_ad_app_amount CHECK(fee_amount>0 AND base_fee_amount>=0 AND regional_fee_amount>=0 AND province_target_count>=0 AND district_target_count>=0);
