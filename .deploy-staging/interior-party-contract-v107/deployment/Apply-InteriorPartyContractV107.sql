...
BEGIN TRY
 ...
 BEGIN
  IF OBJECT_ID(N'dbo.interior_projects',N'U') IS NULL OR OBJECT_ID(N'dbo.interior_contracts',N'U') IS NULL OR OBJECT_ID(N'dbo.provider_wallets',N'U') IS NULL OR OBJECT_ID(N'dbo.wallet_ledger',N'U') IS NULL THROW 51700,'V107 requires interior, wallet, and ledger tables.',1;
  ...
  UPDATE dbo.interior_contracts SET contract_signed_date=CONVERT(date,created_at),registered_by_provider_at=created_at WHERE contract_signed_date IS NULL OR registered_by_provider_at IS NULL;
  ALTER TABLE dbo.interior_contracts ALTER COLUMN contract_signed_date date NOT NULL;
  ALTER TABLE dbo.interior_contracts ALTER COLUMN registered_by_provider_at datetime2 NOT NULL;
  ...
  END;
  IF NOT EXISTS(SELECT 1 FROM sys.foreign_keys WHERE name=N'FK_interior_projects_provider_wallets_fee_reservation_wallet_id') ALTER TABLE dbo.interior_projects ADD CONSTRAINT FK_interior_projects_provider_wallets_fee_reservation_wallet_id FOREIGN KEY(fee_reservation_wallet_id) REFERENCES dbo.provider_wallets(id);
  IF NOT EXISTS(SELECT 1 FROM sys.foreign_keys WHERE name=N'FK_interior_projects_wallet_ledger_fee_reservation_ledger_entry_id') ALTER TABLE dbo.interior_projects ADD CONSTRAINT FK_interior_projects_wallet_ledger_fee_reservation_ledger_entry_id FOREIGN KEY(fee_reservation_ledger_entry_id) REFERENCES dbo.wallet_ledger(id);
  IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.interior_projects') AND name=N'IX_interior_projects_fee_reservation_wallet_id') CREATE INDEX IX_interior_projects_fee_reservation_wallet_id ON dbo.interior_projects(fee_reservation_wallet_id);
  IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.interior_projects') AND name=N'IX_interior_projects_fee_reservation_ledger_entry_id') CREATE INDEX IX_interior_projects_fee_reservation_ledger_entry_id ON dbo.interior_projects(fee_reservation_ledger_entry_id);
