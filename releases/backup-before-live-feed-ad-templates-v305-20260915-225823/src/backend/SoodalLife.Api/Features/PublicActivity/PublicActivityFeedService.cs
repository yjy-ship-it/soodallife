...
public sealed class PublicActivityFeedService(SoodalLifeDbContext db,IMemoryCache cache,ILogger<PublicActivityFeedService> logger)
{
    ...
    public async Task<PublicActivityFeedResponse> GetAsync(string? region, string? eventType, int take, CancellationToken token)
    {
        ...
        var key=$"public-activity:v289:{normalizedRegion?.ToUpperInvariant()??"ALL"}:{normalizedType??"ALL"}:{normalizedTake}";
    ...
    private async Task<PublicActivityFeedResponse> BuildAsync(string? region, string? eventType, int take, CancellationToken token)
    {
        ...
        var query=from activity in db.PublicActivityEvents.AsNoTracking()
                  ...
                  where activity.OccurredAt>=cutoff&&activity.OccurredAt<=visibleBefore&&!request.IsUrgent&&request.StatusCode!="CANCELLED"&&category.StatusCode=="ACTIVE"&&
                  ...
                  select new{activity.ServiceRequestId,activity.EventTypeCode,activity.OccurredAt,request.Title,request.Description,request.OpenedAt,category.PublicId,category.Name,category.ExternalCode,category.SearchSlug,RegionName=area==null?"전국":parent==null||parent.AreaName==area.AreaName?area.AreaName:parent.AreaName+" "+area.AreaName};
    ...
    private async Task<PublicActivityFeedResponse> BuildLegacyAsync(string? region, string? eventType, int take, CancellationToken token)
    {
        ...
        var requests = await db.ServiceRequests.AsNoTracking()
            .Where(x => requestIds.Contains(x.Id) && !x.IsUrgent && x.StatusCode != "CANCELLED")
            .Select(x => new { x.Id, x.CategoryId, x.AdministrativeAreaId, x.Title, x.Description, x.OpenedAt }).ToListAsync(token);
        ...
        foreach (var request in requests)
        {
            ...
            metadata[request.Id] = new RequestMetadata(request.Id, category.PublicId, category.Name, category.ExternalCode, category.SearchSlug, regionName, request.Title, request.Description, request.OpenedAt);
        }
    ...
    private static PublicActivityItemResponse Map(ActivityCandidate value, RequestMetadata metadata, IReadOnlyList<PublicRequestDetailResponse> requestDetails, IReadOnlyList<PublicQuoteAmountResponse> quoteAmounts)
    {
        ...
        return new PublicActivityItemResponse(Id(value), value.EventTypeCode, label, metadata.ServiceId, metadata.ServiceName, metadata.ServiceCode,
    ...
    private sealed record RequestMetadata(long RequestId, Guid ServiceId, string ServiceName, string? ServiceCode, string? SearchSlug, string RegionName, string Title, string? Description, DateTime? OpenedAt);
