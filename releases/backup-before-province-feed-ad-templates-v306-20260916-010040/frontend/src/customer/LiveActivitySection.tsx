...
const FEED_CACHE_KEY = 'soodal-public-activity:v305'
...
export function LiveActivitySection({ signedIn }: { signedIn: boolean }) {
  ...
  useEffect(() => {
    ...
    void loadDefaultAddress().then(value => {
      if (!active) return
      setFeedRegion(value?.regionLabel ?? '')
