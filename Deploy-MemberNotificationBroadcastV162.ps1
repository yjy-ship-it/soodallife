...
param([string]$ZipPath=(Join-Path $PSScriptRoot 'SoodalLife-MemberNotificationBroadcast-20260830-v162.zip'),[string]$ReleaseBasePath='D:\SOODALLIFE\releases',[switch]$ConfirmProductionDeployment)
$ErrorActionPreference='Stop'
$expectedSha256='5A63C34098015AB0BE482F8DE246C9E8240817293884493A689C6B332E4A7B5D'
...
$stamp=Get-Date -Format 'yyyyMMdd-HHmmss';$extractPath=Join-Path $ReleaseBasePath "member-notification-broadcast-v162-$stamp";New-Item -ItemType Directory -Path $extractPath|Out-Null
...
Write-Host "2/4 Extracted V162 deployment validated: $release"
...
Write-Host '4/4 V162 registered member broadcast deployment completed.' -ForegroundColor Green
Write-Host 'Broadcasts are limited to registered members, deduplicated and processed in batches of 200 with final consent revalidation.' -ForegroundColor Yellow
Write-Host 'KAKAO is restricted to informational notices; external contact upload is not included.' -ForegroundColor Yellow
