...
param(
    [string]$ZipPath=(Join-Path $PSScriptRoot 'SoodalLife-CustomerHomeDiversifiedFooter-20260829-v158.zip'),
    [string]$ReleaseBasePath='D:\SOODALLIFE\releases',
    [switch]$ConfirmProductionDeployment
)
$ErrorActionPreference='Stop'
$expectedSha256='1CE6C622B835EB59E2ED4C83DEF8DA447F893E1E2C474B48820F026407ABA7EC'
...
$stamp=Get-Date -Format 'yyyyMMdd-HHmmss';$extractPath=Join-Path $ReleaseBasePath "customer-home-diversified-footer-v158-$stamp";New-Item -ItemType Directory -Path $extractPath|Out-Null
...
$required=@((Join-Path $release 'api\SoodalLife.Api.dll'),(Join-Path $release 'api\web.config'),(Join-Path $release 'frontend\customer\index.html'),(Join-Path $release 'frontend\partner\index.html'),(Join-Path $release 'frontend\admin\index.html'),(Join-Path $release 'RELEASE-MANIFEST.txt'))
...
Write-Host "2/4 Extracted V158 deployment validated: $release"
...
Write-Host '4/4 V158 customer home and mobile footer deployment completed.' -ForegroundColor Green
